"""Legacy video processing helpers bridged onto OSCC_postprocessing where possible."""
...
from OSCC_postprocessing.cine.functions_videos import subtract_median_background
...
def removeBackgroundSimple(video, first_frame, threshold=10):
    ...
    reference = np.asarray(first_frame)
    for i in range(video.shape[0]):
        diff = cv2.absdiff(video[i], reference)
        _, mask = cv2.threshold(diff, threshold, 255, cv2.THRESH_BINARY)
        out[i] = cv2.bitwise_and(video[i], video[i], mask=mask)
    return out
...
def createRotatedVideo(video, angle):
    return rotate_video_auto(video, angle=angle)
...
def applyLaplacianFilter(video):
    kernel = build_2d_kernel("laplacian", 3)
    filtered = convolve_video_2d(video, kernel, backend="auto")
    return cv2.convertScaleAbs(np.asarray(filtered))

def applyDoGfilter(video, ksize1=5, ksize2=9):
    video_f32 = np.asarray(video, dtype=np.float32)
    blur1 = _gaussian_video_cpu(video_f32, ksize=(ksize1, ksize1), sigma=0)
    blur2 = _gaussian_video_cpu(video_f32, ksize=(ksize2, ksize2), sigma=0)
    return cv2.convertScaleAbs(blur1 - blur2)
...
def temporalMedianFilter(video, firstFrameNumber):
    foreground, _background = subtract_median_background(
        np.asarray(video, dtype=np.float32),
        frame_range=slice(firstFrameNumber, firstFrameNumber + 30),
    )
    return np.abs(foreground).astype(video.dtype, copy=False)
...
def Gaussian_LP_video(video_array, cutoff, max_workers=None):
    """Delegate Gaussian low-pass filtering to OSCC_postprocessing with safe fallback."""
    main_file = getattr(__main__, "__file__", None)
    try:
        if main_file and main_file != "<stdin>":
            result = _Gaussian_LP_video(video_array, cutoff, max_workers=max_workers)
            result_arr = np.asarray(result)
            if result_arr.shape == np.asarray(video_array).shape:
                return result_arr
    except Exception:
        pass

    out = np.empty_like(video_array, dtype=np.float64)
    for i in range(video_array.shape[0]):
        out[i] = _gaussian_low_pass_frame(video_array[i], cutoff)
    return out
