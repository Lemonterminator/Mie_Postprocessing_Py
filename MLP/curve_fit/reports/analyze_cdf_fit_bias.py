...
The script writes audit CSVs and figures under
``MLP/figures/fit_bias_audit_cdf`` by default. It does not refit the dataset;
it only reconstructs and validates the existing fit pipeline outputs.
"""
...
CURVE_FIT_DIR = THIS_DIR.parent
MLP_DIR = CURVE_FIT_DIR.parent
if str(CURVE_FIT_DIR) not in sys.path:
    sys.path.insert(0, str(CURVE_FIT_DIR))
...
DEFAULT_OUT_DIR = MLP_DIR / "figures" / "fit_bias_audit_cdf"
TIME_BIN_MS = 0.10
MAX_TIME_MS = 5.0
RELATIVE_EPS = 1e-9
...
def build_inventory(
    ...
    *,
    ...
    """Load saved CDF fit rows and wide cleaned series for the requested slice."""
...
def load_file_bundle(file_path: str, dataset: str, cache: dict[str, dict]) -> dict:
    """Read one raw CSV and cache the cleaned CDF traces produced by fit_raw_data."""
...
def trace_cdf_pipeline(bundle: dict, plume_idx: int) -> dict:
    """Replay per-plume CDF preprocessing and count how each filter changes it."""
    ...
    # ``used_delay`` is the clipped/aligned delay exported by fit_raw_data; the
    # raw-delay branch below mirrors the intermediate filter-stage decisions.
    ...
    if positive_idx.size == 0:
        ...
        lower_cut_removed_count = 0
        # Lower-difference cutoff marks the first plateau / stalled growth
        # point, which is where the fitting trace is truncated.
        ...
        upper_cut_removed_count = 0
        # Upper-difference cutoff removes late jumps that are usually caused by
        # segmentation spikes rather than physical spray growth.
...
def compare_trace_to_wide(
    ...
    """Check that reconstructed cleaned traces match ``series_wide_all`` rows."""
...
def build_fit_curve(trace_time_ms: np.ndarray, fit_row: pd.Series) -> np.ndarray:
    """Evaluate the stored sigmoid/quarter-root fit on reconstructed time points."""
...
def assemble_audit_tables(
    ...
    """Build plume-level audit rows plus point-level processed-vs-fit residuals."""
