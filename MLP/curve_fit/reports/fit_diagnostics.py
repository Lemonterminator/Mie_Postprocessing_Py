...
The functions are intentionally side-effect-light: each diagnostic section
returns a small manifest of the files it wrote, and ``run_all`` assembles the
...
def _aggregate_series_clean(pen_key: str = DEFAULT_PEN_KEY) -> pd.DataFrame:
    """Read every ``series_clean`` point table into one long DataFrame."""
    frames = []
    for name in frd.names:
