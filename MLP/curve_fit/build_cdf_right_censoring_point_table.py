...
def write_plots(
    *,
    ...
    ax.scatter(unc["time_ms"], unc["penetration_mm"], s=1.8, alpha=0.16, linewidths=0, color="#4c78a8", label=f"kept (n={len(unc):,})")
    ax.scatter(cen["time_ms"], cen["penetration_mm"], s=1.8, alpha=0.18, linewidths=0, color="#e45756", label=f"right-censored (n={len(cen):,})")
    ax.set_xlabel("Time [ms]")
    ax.set_ylabel("CDF penetration [mm]")
...
def main() -> None:
    ...
    print(f"Wrote CDF right-censoring point tables to: {out_dir}")
    print(json.dumps({
