...
if __package__ is None or __package__ == "":
    ...
    from models import params_to_dict
...
def apply_variant_preset(config: dict[str, Any], variant: str | None) -> dict[str, Any]:
    ...
    if variant == "ns_no_angle":
        config["fit_delay"] = False
        config["use_train_angle"] = False
        config["use_oracle_angle"] = False
    elif variant == "ns_delay":
        config["fit_delay"] = True
        config["use_train_angle"] = False
        config["use_oracle_angle"] = False
    elif variant == "ns_train_angle":
        config["fit_delay"] = True
        config["use_train_angle"] = True
        config["use_oracle_angle"] = False
    elif variant == "ns_oracle_angle":
        config["fit_delay"] = True
        config["use_train_angle"] = False
        config["use_oracle_angle"] = True
    else:
...
def resolve_output_root(config: dict[str, Any], override: Path | None) -> Path:
    root = override or Path(config.get("outputs", {}).get("root", "MLP/baseline/Naber_Siebers/outputs"))
    root = Path(root)
...
def maybe_subsample_wide(wide: pd.DataFrame, max_trajectories: int | None, seed: int) -> pd.DataFrame:
    ...
    if len(out) < int(max_trajectories):
        ...
        if not remaining.empty:
            out = pd.concat(
                [out, remaining.sample(n=min(int(max_trajectories) - len(out), len(remaining)), random_state=int(seed) + 1)],
                sort=False,
            )
...
def run_once(
    ...
    *,
    ...
    train_points_for_angles = points.loc[points["sample_split"] == "train"].reset_index(drop=True)
    angle_lookup = fit_angle_lookup(train_points_for_angles, default_angle_deg=float(config.get("angle_default_deg", 14.0)))
    points = apply_angle_policy(points, config, angle_lookup)

    train_points = points.loc[points["sample_split"] == "train"].reset_index(drop=True)
    params, fit_info = fit_naber_siebers(train_points, config)
    ...
    boot_params, boot_df = bootstrap_params(train_points, config)
    ...
    name_parts = [stamp, str(config.get("variant", "ns")), str(config.get("split_mode", "split"))]
    ...
    effective_config["angle_lookup"] = angle_lookup
    effective_config["uncertainty_calibration_split_used"] = cal_split
    write_json(run_dir / "config_effective.json", effective_config)
    ...
    if should_plot:
        save_standard_plots(
            ...
            time_bins=time_bins,
            out_dir=run_dir,
            title_prefix=f"Naber--Siebers {config.get('variant')} ({primary_split})",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Fit and evaluate a Naber--Siebers penetration baseline.")
    p.add_argument("--config", type=Path, default=None, help="Optional JSON config override.")
    p.add_argument("--variant", choices=("ns_no_angle", "ns_delay", "ns_train_angle", "ns_oracle_angle"), default=None)
    ...
    p.add_argument("--max-trajectories", type=int, default=None, help="Smoke-test subset size at trajectory level.")
...
def main() -> None:
    ...
    print(f"Wrote Naber--Siebers baseline to {run_dir}")
