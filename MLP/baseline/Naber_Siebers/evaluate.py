...
try:
    ...
    from .models import NSParams, predict
except ImportError:  # pragma: no cover
    from fit import ResidualUncertainty, parameter_sigma_for_points, residual_sigma_for_points
    from models import NSParams, predict


def finite_metrics(truth: np.ndarray, pred: np.ndarray, std: np.ndarray, rel_floor: float) -> dict[str, float | int]:
    resid = pred - truth
    ...
    return {
        ...
        "nrmse_range": float(np.sqrt(np.mean(resid**2)) / truth_range) if truth.size and truth_range > 0 else float("nan"),
...
def build_predictions(
    ...
    sigma_param = parameter_sigma_for_points(out, boot_params, config)
...
def time_bin_metrics(points_df: pd.DataFrame, bin_ms: float = 0.1) -> pd.DataFrame:
    df = points_df.copy()
    df["time_bin"] = np.floor(pd.to_numeric(df["time_ms"], errors="coerce") / float(bin_ms)).astype(int)
    rows: list[dict[str, Any]] = []
    for b, g in df.groupby("time_bin", dropna=False):
        ...
        rows.append(
            {
                "time_bin": int(b),
                "time_bin_center_ms": (int(b) + 0.5) * float(bin_ms),
                **m,
            }
        )
    return pd.DataFrame(rows)
...
def evaluate_split(
    ...
    *,
    ...
    pred_points = build_predictions(split_points, params, residual_unc, boot_params, config)
    ...
    time_bins = time_bin_metrics(pred_points, bin_ms=float(config.get("uncertainty", {}).get("time_bin_ms", 0.1)))
