...
def load_source_table(source: str = "cdf", split: str = "clean", synthetic_root: Path | None = None) -> pd.DataFrame:
    ...
    wide_dir_name, _long_dir_name = split_dir_names(split)
    ...
    for experiment_dir in sorted(root.iterdir()):
        ...
        wide_dir = source_dir / wide_dir_name
...
def add_canonical_features(df_in: pd.DataFrame, registry: dict[str, DatasetMeta]) -> pd.DataFrame:
    ...
    df["A_scale"] = np.power(df["delta_pressure_bar_phys"] / df["ambient_density_kg_m3"], 0.25) * np.sqrt(
        df["diameter_mm"].clip(lower=1e-12)
    )
    return df
...
def resolve_repo_path(path_text: str) -> Path:
    """Resolve Windows paths stored in CSV exports against the current repo."""
    text = str(path_text)
    if not text:
        return Path(text)
...
def enrich_observed_angles(wide_df: pd.DataFrame, default_angle_deg: float = 14.0) -> pd.DataFrame:
    df = wide_df.copy()
    cache: dict[str, float] = {}

    def read_angle(path_text: str) -> float:
        if not isinstance(path_text, str) or not path_text:
            return float(default_angle_deg)
        if path_text in cache:
            return cache[path_text]
        path = resolve_repo_path(path_text)
        candidates = [path.with_suffix(".meta.json")]
        if not path.is_absolute():
            candidates.append(PROJECT_ROOT / path.with_suffix(".meta.json"))
        ...
        for meta_path in candidates:
            ...
            try:
                ...
                continue
            value = payload.get("cone_angle_proxy_deg", default_angle_deg)
            try:
                ...
                pass
        cache[path_text] = angle
        return angle

    if "observed_cone_angle_deg" not in df.columns:
        df["observed_cone_angle_deg"] = [read_angle(str(path)) for path in df.get("file_path", pd.Series("", index=df.index))]
    return df
...
def prepare_wide_table(config: dict[str, Any]) -> pd.DataFrame:
    ...
    wide["test_name"] = [
        ...
        for path, fallback in zip(wide.get("file_path", ""), wide.get("file_stem", wide.index.astype(str)))
    ]
    wide["traj_key"] = (
    ...
    wide = enrich_observed_angles(wide, default_angle_deg=float(config.get("angle_default_deg", 14.0)))
