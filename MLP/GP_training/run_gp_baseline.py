...
class PointTensorDataset(Dataset):
    """Flattened point dataset used by SVGP training.

    RESIDUAL_SVGP note: ``family_id`` is stored beside the features so residual
    multi-task models can route to delta GPs.  It is not appended to
    ``features`` and therefore does not become an ordinary kernel dimension.
    """
    ...
    def __init__(
        ...
        if family_id is None:
            self.family_id = torch.zeros(len(self.features), dtype=torch.long)
        else:
            self.family_id = family_id.detach().cpu().reshape(-1).contiguous().long()
        if not (
            len(self.features)
...
def _family_ids_from_frame(df: pd.DataFrame) -> np.ndarray:
    """Derive Nozzle0-vs-modified family labels from existing row metadata."""

    if "family_id" in df.columns:
        values = pd.to_numeric(df["family_id"], errors="coerce").fillna(1).to_numpy(dtype=np.int64)
        return values
    name_source = next(
        (col for col in ("experiment_name", "source_dataset_name", "folder", "dataset_key") if col in df.columns),
        None,
    )
    if name_source is None:
        return np.zeros(len(df), dtype=np.int64)
    return df[name_source].astype(str).map(family_id_from_name).to_numpy(dtype=np.int64)
...
def flatten_curve_dataset(curve_ds: PointwisePenetrationDataset) -> PointTensorDataset:
    ...
    family_curve = torch.from_numpy(_family_ids_from_frame(curve_ds.df))
    family_point = family_curve[:, None].expand(-1, curve_ds.n_points).reshape(-1)
    return PointTensorDataset(
        features,
...
def concat_point_datasets(*datasets: PointTensorDataset | None) -> PointTensorDataset:
    ...
    return PointTensorDataset(
        ...
        torch.cat([d.family_id for d in items], dim=0),
    )
...
def build_real_cdf_point_datasets(
    *,
    ...
    is_val_row = rng.random(len(cdf)) < float(val_ratio)
    if len(cdf) > 1:
        if not bool(is_val_row.any()):
            is_val_row[int(rng.integers(0, len(cdf)))] = True
        if bool(is_val_row.all()):
            is_val_row[int(rng.integers(0, len(cdf)))] = False
    if max_rows is not None and len(cdf) > int(max_rows):
        max_rows_int = max(1, int(max_rows))
        all_idx = np.arange(len(cdf))
        if max_rows_int >= 2 and bool(is_val_row.any()) and bool((~is_val_row).any()):
            train_idx = all_idx[~is_val_row]
            val_idx = all_idx[is_val_row]
            target_val = max(1, min(len(val_idx), int(round(max_rows_int * float(val_ratio)))))
            target_train = max(1, max_rows_int - target_val)
            if target_train > len(train_idx):
                target_train = len(train_idx)
                target_val = min(len(val_idx), max_rows_int - target_train)
            if target_val < 1:
                target_val = 1
                target_train = min(len(train_idx), max_rows_int - target_val)
            selected = np.concatenate([
                rng.choice(train_idx, size=target_train, replace=False),
                rng.choice(val_idx, size=target_val, replace=False),
            ])
            if selected.size < max_rows_int:
                remaining = np.setdiff1d(all_idx, selected, assume_unique=False)
                fill = rng.choice(
                    remaining,
                    size=min(max_rows_int - selected.size, remaining.size),
                    replace=False,
                )
                selected = np.concatenate([selected, fill])
        else:
            selected = rng.choice(all_idx, size=max_rows_int, replace=False)
        selected = np.sort(selected.astype(int))
        cdf = cdf.iloc[selected].reset_index(drop=True)
        is_val_row = is_val_row[selected]
    n_rows = len(cdf)
    ...
    fam_train: list[np.ndarray] = []
    fam_val: list[np.ndarray] = []
    n_rows_used = 0
    ...
    for row_idx, row in cdf.iterrows():
        ...
        family_value = int(row["family_id"]) if "family_id" in row.index and pd.notna(row["family_id"]) else family_id_from_name(str(row.get("experiment_name", "")))
        family_arr = np.full(len(target_scaled), family_value, dtype=np.int64)

        if is_val_row[row_idx]:
            feat_val.append(features_np); ts_val.append(target_scaled)
            tp_val.append(target_phys); as_val.append(a_scale_flat); fam_val.append(family_arr)
        else:
            feat_train.append(features_np); ts_train.append(target_scaled)
            tp_train.append(target_phys); as_train.append(a_scale_flat); fam_train.append(family_arr)
        n_rows_used += 1

    def _stack(feat, ts, tp, asc, fam) -> PointTensorDataset | None:
        if not feat:
            return None
        return PointTensorDataset(
            ...
            torch.from_numpy(np.concatenate(fam).astype(np.int64)),
        )

    train_ds = _stack(feat_train, ts_train, tp_train, as_train, fam_train)
    val_ds = _stack(feat_val, ts_val, tp_val, as_val, fam_val)
    ...
    info = {
        ...
        "n_rows_after_holdout": int(n_rows_after_holdout),
        "max_rows": int(max_rows) if max_rows is not None else None,
        "n_rows_after_max_rows": int(n_rows),
...
class SVGPModel(gpytorch.models.ApproximateGP):
    def __init__(self, inducing_points: torch.Tensor, *, num_dims: int, mean_type: str = "constant"):
        ...
        # RESIDUAL_SVGP uses ZeroMean for delta GPs so untrained residuals
        # express "no offset" relative to the hot-started shared model.
        if str(mean_type) == "zero":
            self.mean_module = gpytorch.means.ZeroMean()
        elif str(mean_type) == "constant":
            self.mean_module = gpytorch.means.ConstantMean()
        else:
            raise ValueError(f"Unsupported SVGP mean_type={mean_type!r}.")
