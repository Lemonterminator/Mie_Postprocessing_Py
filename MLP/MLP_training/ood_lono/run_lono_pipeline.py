...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--variant", default="a_only",
                   help="Stage-1 feature variant to train/evaluate (default: a_only).")
    # Passthrough to Stage 1 / Stage 2 for Tier-1A architecture sweeps.
    p.add_argument("--hidden-dims", type=str, default=None,
                   help="Comma-separated hidden layer widths, e.g. '256,256,64'. "
                        "Forwarded to Stage 1 and Stage 2 (must match).")
    # Tier-1C lr/wd passthrough. Stage 1 owns the meaningful sweep; Stage 2
    # keeps its own defaults unless --stage2-learning-rate / --stage2-weight-decay
    # are also set.
    p.add_argument("--learning-rate", type=float, default=None,
                   help="Stage-1 learning rate (Tier-1C). Stage 2 keeps its default unless --stage2-learning-rate is set.")
    p.add_argument("--weight-decay", type=float, default=None,
                   help="Stage-1 weight decay (Tier-1C). Stage 2 keeps its default unless --stage2-weight-decay is set.")
    p.add_argument("--stage2-learning-rate", type=float, default=None,
                   help="Override Stage-2 learning rate; defaults to its own DEFAULT_STAGE2_CONFIG value if unset.")
    p.add_argument("--stage2-weight-decay", type=float, default=None,
                   help="Override Stage-2 weight decay; defaults to its own DEFAULT_STAGE2_CONFIG value if unset.")
    # Tier-2C onset aux head (forwarded to both Stage 1 and Stage 2).
    p.add_argument("--onset-aux-head", action="store_true",
                   help="Train with the onset auxiliary regression head (Tier 2C).")
    p.add_argument("--lambda-aux", type=float, default=None,
                   help="Onset aux MSE weight (Tier 2C). Default 0.1 once --onset-aux-head is on.")
    p.add_argument("--onset-aux-hidden", type=int, default=None,
                   help="Aux head hidden width (default: 64).")
    p.add_argument("--onset-t-ms-max", type=float, default=None,
                   help="Onset slice cutoff in ms (default: 0.3).")
    # Tier-3A family-aware architecture + LONO protocol selection.
    p.add_argument("--architecture-mode", choices=("single", "family_head", "residual_family_head",
                                                   "residual_film_last_block", "residual_film_all_blocks"), default="single",
                   help="single: production PenetrationMLP. family_head: FamilyAwarePenetrationMLP (Tier 3A). "
                        "residual_family_head: shared mu plus family residual deltas. "
                        "residual_film_*: residual head plus identity FiLM adapters.")
    p.add_argument("--n-families", type=int, default=None,
                   help="Number of family routing heads (default: 2).")
    p.add_argument("--protocol", choices=("lono_5fold", "lono_modified_only"), default="lono_5fold",
                   help="lono_5fold: hold out each of Nozzle0..5 in turn. lono_modified_only: skip Nozzle0 (Tier 3A).")
    # Tier-3B learnable per-nozzle lambda_d2 (forwarded to Stage 1 only).
    p.add_argument("--learnable-d2", action="store_true",
                   help="Stage-1 learns a per-nozzle lambda_d2 (Tier 3B). Forwards to Stage 1; Stage 2 inherits.")
    p.add_argument("--n-families-for-d2", type=int, default=None,
                   help="Number of learnable lambda_d2 (default: 6, one per nozzle).")
    p.add_argument("--d2-concave-weight", type=float, default=None,
                   help="Override the scalar d2_concave_weight (set 0 for Tier 3B no_d2_penalty).")
    p.add_argument("--dry-run", action="store_true")
    return p.parse_args()


def _build_passthrough_dicts(args: argparse.Namespace) -> tuple[dict[str, Any], dict[str, Any]]:
    """Collect CLI flags that should be forwarded into Stage 1 / Stage 2.

    Stage 1 and Stage 2 receive the same set of flags by default — they share
    most CLI surface (--hidden-dims, --learning-rate, --weight-decay, the
    Tier-2C aux head, the Tier-3A/3B architecture switches, etc.). Callers
    that need stage-specific values can mutate the returned dicts before
    they hit run_one_fold().
    """
    shared: dict[str, Any] = {}
    if getattr(args, "hidden_dims", None) is not None:
        shared["hidden_dims"] = args.hidden_dims

    stage1 = dict(shared)
    stage2 = dict(shared)

    # Tier-1C: Stage 1 owns the main lr/wd sweep; Stage 2 only gets its own
    # values if the caller explicitly asks for them via --stage2-*.
    if getattr(args, "learning_rate", None) is not None:
        stage1["learning_rate"] = float(args.learning_rate)
    if getattr(args, "weight_decay", None) is not None:
        stage1["weight_decay"] = float(args.weight_decay)
    if getattr(args, "stage2_learning_rate", None) is not None:
        stage2["learning_rate"] = float(args.stage2_learning_rate)
    if getattr(args, "stage2_weight_decay", None) is not None:
        stage2["weight_decay"] = float(args.stage2_weight_decay)

    # Tier-2C aux head: forwarded to both stages (must match for warm-start).
    if getattr(args, "onset_aux_head", False):
        stage1["onset_aux_head"] = True
        stage2["onset_aux_head"] = True
    if getattr(args, "lambda_aux", None) is not None:
        stage1["lambda_aux"] = float(args.lambda_aux)
        stage2["lambda_aux"] = float(args.lambda_aux)
    if getattr(args, "onset_aux_hidden", None) is not None:
        stage1["onset_aux_hidden"] = int(args.onset_aux_hidden)
    if getattr(args, "onset_t_ms_max", None) is not None:
        stage1["onset_t_ms_max"] = float(args.onset_t_ms_max)
        stage2["onset_t_ms_max"] = float(args.onset_t_ms_max)

    # Tier-3A family-aware architecture (Stage 2 inherits; we forward to both
    # for explicitness so the Stage-2 sanity check passes).
    arch_mode = getattr(args, "architecture_mode", "single") or "single"
    if arch_mode != "single":
        stage1["architecture_mode"] = arch_mode
        stage2["architecture_mode"] = arch_mode
    if getattr(args, "n_families", None) is not None:
        stage1["n_families"] = int(args.n_families)
        stage2["n_families"] = int(args.n_families)

    # Tier-3B learnable lambda_d2: Stage 1 owns the learnable parameters;
    # Stage 2 inherits via its own extra_overrides scan of the Stage-1 config.
    if getattr(args, "learnable_d2", False):
        stage1["learnable_d2"] = True
    if getattr(args, "n_families_for_d2", None) is not None:
        stage1["n_families_for_d2"] = int(args.n_families_for_d2)
    if getattr(args, "d2_concave_weight", None) is not None:
        stage1["d2_concave_weight"] = float(args.d2_concave_weight)
    return stage1, stage2


def _append_passthrough(cmd: list[str], passthrough: dict[str, Any]) -> None:
    """Append passthrough flags (k -> v) to a subprocess command list.

    Boolean True becomes a bare flag; False/None is skipped; everything else
    becomes "--key value". Keys are converted underscore -> dash.
    """
...
def run_one_fold(
    *,
    ...
    stage3_only: str,
    variant: str,
    stage1_passthrough: dict[str, Any] | None = None,
    stage2_passthrough: dict[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    _append_passthrough(s1_cmd, stage1_passthrough or {})
    t0 = time.time()
    if dry_run:
    ...
    _append_passthrough(s2_cmd, stage2_passthrough or {})
    t0 = time.time()
    if dry_run:
...
def main() -> None:
    ...
    # Tier-3A: lono_modified_only filters out Nozzle0 so it always lives in
    # the training set. lono_5fold is the unchanged default.
    if getattr(args, "protocol", "lono_5fold") == "lono_modified_only":
        before = list(nozzles)
        nozzles = [n for n in nozzles if "Nozzle0" not in n]
        dropped = [n for n in before if n not in nozzles]
        if dropped:
            print(f"[protocol=lono_modified_only] Dropping folds: {dropped}")
    if args.n_folds is not None and args.n_folds < len(nozzles):
        nozzles = nozzles[: args.n_folds]
    print(f"\nWill run {len(nozzles)} folds: {nozzles}\n")

    stage1_passthrough, stage2_passthrough = _build_passthrough_dicts(args)
    (output_dir / "lono_config.json").write_text(json.dumps({
        ...
        "stage1_passthrough": stage1_passthrough,
        "stage2_passthrough": stage2_passthrough,
