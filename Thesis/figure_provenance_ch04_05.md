...
| 6 | `fig_sparse_support_topology.png` | 数据来自 `MLP/gradient_stability_diagnostics.py`；出图入口 `Thesis/slides/legacy_notebook_sources/slides_sparse_feature_instability/export_sparse_feature_instability_figures.py:234`（写 `support_topology.png`，后改名加 `fig_` 前缀） | `python …/export_sparse_feature_instability_figures.py` | (chamber,injection) / (chamber,backpressure) 网格上稀疏支撑计数热图 | 数据筛查 |
| 7 | `fig_sparse_diameter_interpolants.png` | 同上脚本 `:246`（写 `diameter_interpolants.png`，后改名） | 同上 | 稀疏直径支撑下 MLP 在 t=0.85ms 的非单调插值/梯度伪影 | 数据筛查 |
...
| 26 | `residual_family_head_20260608/injector_comparison_dp050.png` | `MLP/MLP_training/ablations/compare_campaign_penetration.py:218` (`plot_comparison`) | `python … compare_campaign_penetration.py --dp-exp 0.50 --output-dir …` | 无量纲 S* vs t*(动量标度 dp_exp=0.5)参考 vs 其他喷油器残差偏置 | 当前模型 |
| 27 | `residual_family_head_20260608/injector_scatter_all_nozzles.png` | `MLP/MLP_training/ablations/compare_campaign_penetration.py:250` (`plot_raw_scatter`，脚本内名 `scatter_all_nozzles.png`，部署时改名) | 同上 | 各 campaign 无量纲点按喷嘴着色散点 | 当前模型 |
