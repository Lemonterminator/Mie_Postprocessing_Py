...
\begin{itemize}
  ...
  \item d2 gate 回退能帮助部分 HZ nozzle，但会损失 Nozzle0 和 overall RMSE。
\end{itemize}
\end{column}
\end{columns}
\end{frame}

\begin{frame}{最佳模型如何改变 Nozzle0}
\begin{columns}[T,onlytextwidth]
...
\footnotesize
\begin{itemize}
  \item 最佳 run 明显改善 Nozzle0、Nozzle5/6/7/8。
...
\begin{enumerate}
  ...
  \item \goodtext{主要系统性问题被削弱}：global bias 从 \SI{-1.29}{mm} 改善到 \SI{-0.40}{mm}；Nozzle0 bias 从 \SI{-3.36}{mm} 改善到 \SI{-2.35}{mm}。
  \item \notetext{Ablation 支持 KD-dominance 假设}：deterministic raw-supported bins 中过强 KD 会把 teacher bias 传给 student；降低 KD 并恢复 raw supervision 能改善 Nozzle0。
  \item \warntext{剩余误差具有喷嘴几何依赖性}：Nozzle3 和部分 HZ nozzles 与 Nozzle0 之间存在 trade-off，说明 early-SOI acceleration 和 curvature prior 可能需要 nozzle-aware 设计。
\end{enumerate}
