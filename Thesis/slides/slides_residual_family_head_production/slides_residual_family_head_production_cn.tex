...
\footnotesize
\centering
左：每族 decoder。中：共享均值 + 零初始化残差。右：trunk 内恒等 FiLM + residual head。
绿色模块是可训练参数；灰色模块从部署模型热启动并冻结。
\end{frame}
...
{\scriptsize \textbf{架构} —— 冻结骨干 $+$ 零初始化的逐族残差（Family Head / Residual / FiLM）；未见族 $\to$ 共享模型。}
