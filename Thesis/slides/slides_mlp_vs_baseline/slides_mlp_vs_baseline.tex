...
\footnotesize
The GP uses the same splits, the same five seeds, the same \(A_{\mathrm{scale}}\)
target, the same controllable input features, and the same external clean
diagnostic set as the MLP comparison.
\end{block}
...
not include that teacher/raw staged training mechanism.
\end{block}
\end{frame}
