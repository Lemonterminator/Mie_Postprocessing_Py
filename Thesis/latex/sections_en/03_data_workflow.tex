...
At the implementation level, this image-processing stage is built as a four-lane overlapped pipeline---disk prefetch, GPU compute, CPU feature extraction, and asynchronous output running concurrently---governed by a resumable checkpoint contract, so that the multi-terabyte archive is reprocessed end-to-end on a single workstation GPU rather than as a serial frame-by-frame script (Section~\ref{subsec:concurrency-architecture}).
...
and Figure~\ref{fig:nozzle-geometry} visualizes the geometric design space spanned by these families.
...
\subsubsection{Concurrency Architecture: Batch Scheduling, Memory Reuse, and Recovery Contract}\label{subsec:concurrency-architecture}

This CUDA pipeline is not a simple port of the frame-by-frame MATLAB script, but an end-to-end archive-processing scheduler in which four execution lanes---disk prefetch, GPU compute, CPU feature extraction, and asynchronous output---overlap behind a pool-reuse memory strategy and a resumable recovery contract (Figure~\ref{fig:ch3-pipeline-lanes}).
...
\bottomrule
\end{tabular}
\caption{Measured per-recording wall-clock breakdown, averaged over the full archive of 2{,}992 recordings processed on a single GPU (logged by \codepath{mie\_multi\_hole.py}). GPU stages dominate at ${\sim}84\%$; the asynchronous CSV, metadata, and boundary writes contribute ${<}0.1\%$ to the main-thread critical path because they are submitted as futures to the I/O lane and overlap the next recording's GPU work. The per-recording times sum to ${\approx}72$ minutes (${\approx}42$ recordings/min) for the full cropped archive.}
...
The \textbf{rank-1 separability test} runs a singular-value decomposition, detects the rank-1 structure, and switches to the separable path, reducing the per-pixel multiply-add complexity for a \(k\times k\) kernel from \(O(k^2)\) to \(O(k)\). The two 1D passes are dispatched as CuPy library convolutions (\texttt{cupyx.scipy.ndimage}) rather than hand-written CUDA kernels, so the speed-up follows from the reduced arithmetic complexity of the separable form rather than from custom shared-memory tiling.
...
Quantitative downstream evidence for these CDF-based observables---in particular leave-one-nozzle-family-out (LONO) transfer diagnostics and censoring-controlled prediction behaviour---is presented in Chapter~5.
