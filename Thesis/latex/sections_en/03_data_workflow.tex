...
\smallskip\noindent\textbf{Contributions of this chapter.} Concretely, this chapter contributes five reusable methodological elements to the image-to-observable pipeline: (1)~a log-ratio (homomorphic) foreground estimator that treats glare and non-uniform illumination as multiplicative effects and removes them as additive offsets in log space (Section~\ref{subsec:background-normalization}); (2)~a rank-1 separable Sobel edge-enhancement stage kept entirely on the GPU as two one-dimensional passes (Section~\ref{subsec:edge-enhancement}); (3)~an FFT harmonic-phase alignment that converts the known multi-hole count into an automatic, per-recording plume registration rule rather than a hand-set reference angle (Section~\ref{subsec:plume-registration}); (4)~a cumulative-distribution (CDF) penetration front that is robust to fragmented leading-edge pixels while remaining consistent with standard diesel-spray penetration conventions (Section~\ref{subsec:penetration-measurement}); and (5)~a four-lane overlapped GPU pipeline with a resumable checkpoint contract that reprocesses the multi-terabyte archive end-to-end on a single workstation GPU (Section~\ref{subsec:concurrency-architecture}).
...
\caption{Nozzle geometry calibration workflow. (a) A representative frame is loaded for inspection; (b) visible nozzle-edge points are selected for circle fitting; (c) the fitted center, plume guide lines, and annular processing region are overlaid to verify the geometric anchor used for subsequent batch processing.}
...
Calibrating only once and reusing the same center for all frames avoids inter-frame jitter during downstream plume angle analysis. This one-time calibration is deliberately kept as a user-visible operation rather than fully automated: operators can zoom in, select representative frames, and anchor the entire batch process with a physically meaningful nozzle center \cite{Pratt1987CircleFit,Taubin1991CircleFit}.
...
For each recording, the processing chain answers five practical measurement questions: where the nozzle center is, where each plume direction lies, how each plume can be represented in a common local coordinate system, how far the liquid-phase front has penetrated, and which boundary descriptors can be extracted consistently.
The following sections follow this order.
...
Following tail truncation, a valid frame mask is built and isolated short segments are removed using \texttt{remove\_short\_true\_runs(..., min\_len=5)}.
...
In the second recording (bottom), all three metrics instead climb together and flatten at a common $\approx 92$\,mm ceiling---not a physical end of penetration but the spray crossing the finite camera field-of-view boundary, a structural right-censoring of the observation.
...
Quantitative downstream evidence for these CDF-based observables---in particular leave-one-nozzle-out (LONO) transfer diagnostics and censoring-controlled prediction behaviour (right-censoring here meaning that the penetration front reaches the camera field-of-view boundary before injection ends, so the observed trajectory is truncated rather than physically saturated)---is presented in Chapter~5.
