...
The \textbf{rank-1 separability test} automatically verifies this and switches to the separable path when the condition is met, reducing the per-pixel multiply-add overhead for a \(k\times k\) kernel from \(O(2k)\) instead of \(O(k^2)\).
...
The current implementation first applies percentile-based min-max normalization to each plume's \((t,y,x)\) high-pass strip volume, mapping the \(q_{\min}\) and \(q_{\max}\) percentiles to 0 and 1, and clipping values outside the range.
...
The \textbf{multi-hole geometry construction script} then constructs the plume directions
\[
...
The reusable library function defaults to a lower value during interactive testing; all quantitative results herein use the \(q=0.995\) written into the archive by the batch script.
