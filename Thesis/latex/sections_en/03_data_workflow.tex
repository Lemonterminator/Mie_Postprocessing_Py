...
At the implementation level, this image-processing stage is built as a four-lane overlapped pipeline---disk prefetch, GPU compute, central processing unit (CPU) feature extraction, and asynchronous output running concurrently---governed by a resumable checkpoint contract, so that the multi-terabyte archive is reprocessed end-to-end on a single workstation GPU rather than through serial frame-by-frame processing (Section~\ref{subsec:concurrency-architecture}).
