...
\paragraph{Terminology contract.}This chapter and the following two reuse three framing concepts; their operational definitions are pinned here once and not re-explained below:
\begin{itemize}
  \item \emph{Censoring-aware target construction}: field-of-view right-censoring is treated as a source of label-level target bias and is addressed at three levels (fitting, teacher training, and the raw/teacher mixing of student supervision), not only by point-level filtering at evaluation time.
  \item \emph{Conservative coverage}: empirical 1$\sigma$/2$\sigma$ coverages on the held-out set sit above the nominal Gaussian levels (68.3\%/95.4\%). This bias is a deliberate design choice for early-stage engineering screening, not a calibration defect. Over-dispersion does penalise CRPS (because an unnecessarily large $\sigma$ degrades sharpness), and this trade-off is visible in the Chapter~5 results; the engineering rationale for accepting it is that the false-negative cost of missing a genuine impingement event in early screening exceeds the false-positive cost of an overly cautious flag.
  \item \emph{Ablation affordability}: under a master's-level compute budget, the chosen architecture admits a LONO~$\times$~variant~$\times$~seed grid over several orthogonal design axes, so each design decision carries traceable marginal evidence.
\end{itemize}
Subsequent uses of these three terms are to be read against the definitions above.
...
\midrule
Primary artefact & Long-format and wide-format cleaned CDF series tables & Uncensored CDF point table and P50/q1 oracle tables & Per-trajectory q1 fit tables \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Three derived populations read from the cleaned CDF penetration archive. Dataset~\#1 is the closest pointwise measurement record, Dataset~\#2 is the less-biased evaluation ruler, and Dataset~\#3 is the smooth fitted-target archive used by Stage~1 and Stage~2 (Stages~1--3 refer to the three-stage MLP training curriculum introduced in Section~\ref{sec:mlp-curriculum-en}).}
\label{tab:three-derived-populations}
\end{table}

The distinction between the Stage~1 representative trajectory and the P50/q1 oracle is especially important. The Stage~1 row is one real q1-fitted trajectory drawn from Dataset~\#3: within each operating-condition group, the fitted curve is evaluated at \(\SI{5}{ms}\), the group median is computed, and the fitted trajectory whose 5\,ms evaluation value is closest to that median is retained. The P50/q1 oracle is a virtual trajectory built from Dataset~\#2 by taking the per-time-bin median and then fitting q1 to that median trace. The former is a training target and inherits Dataset~\#3's robust-z filtering; the latter is an evaluation reference and never enters training.
...
A prerequisite for both empirical and learned fitting is that every penetration curve shares a common time origin at the start of injection (SOI). Polynomial reduced-order models and neural networks alike are sensitive to the choice of time zero: if individual plume opening events are scattered across different frames, curves from the same operating condition form a systematically shifted family and the model cannot recover the true penetration dynamics.

The learned models do not train directly on the raw exported traces. Instead, the cleaning and fitting routine first cleans and time-aligns them. For each plume, the routine finds the earliest valid penetration segment, takes the first two positive samples after onset, fits a straight line to those early nonzero points, and uses the \(x\)-axis intercept of that line as a subframe opening-delay estimate whenever the fitted slope is positive. If an area-based onset marker is available---where area is the projected spray area obtained by summing foreground pixels in the binarised image produced by the image-processing pipeline for each frame and converting via the pixel-to-physical scale---that frame anchors the delay scan; otherwise it falls back to the first positive penetration frame. The resulting raw delay is then clipped to a small median-centred window across plumes so that one anomalous plume does not over-shift the whole group.

That delay estimate is used in two coupled ways. The penetration array itself is shifted left by the nearest integer number of frames, while the aligned time vector retains the fractional residual through the exact offset subtraction. The pipeline performs a plume-onset registration that preserves subframe timing information before the later reduced-order fit. After alignment, sudden unrealistic jumps are removed, and low-quality fits are flagged through a set of rule-based masks. The residual inter-plume offset after alignment is typically within three frames; hydraulic delay---the time between the electrical command and the first detectable spray---was not formally characterised across operating conditions, but the frame-level scatter corresponds to a duration well below the injection duration at the frame rate used.
...
This engineering treatment is reasonable for the objectives of this thesis, though its scope has an explicit boundary. From an engineering standpoint, compressing cleaned discrete observations into a small set of curve parameters not only smooths measurement noise but also enables reconstruction of $y = f(t)$ at arbitrary time-bin densities and supports extrapolation within a reasonable range---capabilities that discrete observations alone cannot provide. Three points further support the reasonableness: the fit acts as a smooth compressor of noisy trajectories, not a final mechanistic spray law; it preserves curve differentiability for downstream learning; and it delegates operating-condition dependence to the neural network rather than encoding every physical effect into a closed-form expression. The boundary is that this treatment should be understood as a reduced-order surrogate strategy motivated by empirical penetration theory, not as proof that the shifted-time physics have disappeared. The quarter-root-only ablation below makes this distinction explicit.
...
\textbf{Empirical scaling check.} Each fitted trajectory yields three parameter values \((k_{1/4},\,t_0,\,s)\). Because the gating parameters \((t_0,s)\) are constrained such that the sigmoid product converges to approximately unity within the observable time window, the late-time and extrapolated behaviour satisfies \(S(t)\approx k_{1/4}\cdot t^{1/4}\); consequently \(k_{1/4}\) is not an abstract amplitude parameter but the asymptotic linear scaling coefficient of the \(t^{1/4}\) penetration law, directly comparable to the proportionality constant in classical empirical formulae. Regressing it against operating conditions in log-log space therefore tests whether the fit results follow the power-law scaling predicted by classical penetration theory. Fitting
\[
  \log k_{1/4} = a\,\log\Delta P + b\,\log\rho_a + c\,\log d_n + \text{const}
\]
by ordinary least squares across the \(24{,}316\) accepted fits yields exponents \(a\approx 0.52\), \(b\approx -0.23\), \(c\approx 0.26\) with \(R^2\approx 0.27\).

Comparing against classical Hiroyasu--Arai theory reveals different degrees of agreement. The density exponent \(b\approx-0.23\) matches the post-breakup form \(S\propto\Delta P^{1/4}\rho_a^{-1/4}(d_n t)^{1/2}\) almost exactly at \(-1/4\), a positive signal for the physical plausibility of the fitted parameters. The injection-pressure exponent \(a\approx 0.52\) is noticeably steeper than the H--A value of \(1/4\); the reason is not confirmed. Several factors are each consistent with this discrepancy: (i)~limited condition coverage and pressure collinearity in the experimental matrix, (ii)~functional-form bias introduced by the q1 single-branch parameterisation, (iii)~transitional-regime effects in short pilot injections where the momentum-dominated near-field departs from the free-jet entrainment asymptote, and (iv)~the possibility that the \(t^{1/4}\) branch absorbs penetration dynamics beyond the classical free-jet stage. This factor-of-two departure from the H--A expectation is reported as an unresolved empirical observation rather than an explained phenomenon. The diameter exponent \(c\approx 0.26\) sits well below the classical \(1/2\), most likely reflecting collinearity between nozzle diameter and other operating conditions in the experimental matrix (Section~\ref{sec:pointlevel-scaling-verification} confirms this independently). Regressions on the gating parameters \(t_0\) and \(s\) yield \(R^2\) of only \(0.17\) and \(0.14\) respectively --- an expected result, since sigmoid switching parameters are not directly predicted by classical theory and primarily absorb the residual onset-delay variability left after preprocessing alignment.

As a cross-check, regressing \(\log S\) directly on the raw discrete observations at fixed \(\SI{0.1}{ms}\) time bins yields a log-log \(R^2\) of approximately \(0.47\)--\(0.51\) in the mid-trajectory window (\(0.35\)--\(0.75\)~ms), confirming that the three physical variables explain a meaningful fraction of raw penetration variance in the well-sampled regime. The lower \(R^2\approx 0.27\) from the compressed \(k_{1/4}\) parameter reflects integration across the noisy onset and post-injection regimes rather than model failure; the \(\Delta P\) exponent (\(0.44\)--\(0.65\), intermediate between the Bernoulli value of \(0.5\) and the H--A value of \(0.25\)) and the chamber-pressure exponent (\(\approx-0.25\)) are qualitatively consistent across both analyses.

\begin{figure}[t]
\centering
\includegraphics[width=0.96\linewidth]{images/fig_q1_param_scaling.png}
\caption{Empirical scaling of the q1 production parameters \((k_{1/4},t_0,s)\) against \(\Delta P=P_{\mathrm{inj}}-P_{\mathrm{ch}}\), colour-coded by chamber pressure \(P_{\mathrm{ch}}\). Each panel title reports the fitted log-log exponents and \(R^2\) of the regression \(\log y = a\log\Delta P + b\log\rho_a + c\log d_n + \text{const}\). Because the sigmoid gate is constrained to converge to unity within the observable window, \(k_{1/4}\) is the asymptotic linear scaling coefficient of the \(t^{1/4}\) penetration law and can be compared directly against classical proportionality constants: \(\rho_a^{-0.23}\) closely matches the Hiroyasu--Arai expectation of \(-1/4\), while the diameter exponent (\(d_n^{0.26}\)) falls noticeably below the classical \(1/2\); the gating parameters are weakly explained by the same dimensional group.}
\label{fig:q1-param-scaling}
\end{figure}

\textbf{Residual structure and heteroscedasticity.} Diagnosing the temporal distribution of fit residuals reveals the systematic behaviour of the q1 model across different phases of the trajectory, and provides the physical motivation for the three-stage training design and mixed supervision. Reconstructing the residual \(\hat S(t)-y(t)\) for every clean fit at every observed frame, then binning by \(\Delta t=\SI{0.1}{ms}\), produces the empirical envelope in Figure~\ref{fig:q1-residual-structure}, which reveals two qualitatively distinct regimes. In the central window \(t\in[0.3, 1.0]\,\si{ms}\) the median residual stays below \(\SI{0.4}{mm}\) and the robust spread \(\sigma_{\mathrm{robust}}(t)=1.4826\,\mathrm{MAD}(t)\) reaches a minimum of \(\sim\SI{0.60}{mm}\) near \(t=\SI{1.0}{ms}\)---q1 is essentially unbiased and tight in the well-sampled mid-trajectory.

Both boundaries carry systematic bias. The first bin at \(t=\SI{0.05}{ms}\) has a positive bias of \(\sim\SI{+1.7}{mm}\) with \(\sigma_{\mathrm{robust}}\approx\SI{1.8}{mm}\), reflecting onset-alignment imprecision and the steep sigmoid gate transition near \(t_0\). The late tail at \(t=\SI{1.55}{ms}\) has a negative bias of \(\sim\SI{-1.6}{mm}\) with \(\sigma_{\mathrm{robust}}\approx\SI{0.7}{mm}\), reflecting right-censoring of the surviving population---per-bin counts collapse from \(\sim 65{,}000\) frames at \(t=\SI{0.15}{ms}\) to fewer than \(200\) at \(t=\SI{1.55}{ms}\). The envelope is therefore U-shaped rather than monotonically growing. This boundary bias is the core motivation for introducing mixed supervision in the three-stage training: the q1 fitting target carries structural errors at both the early-injection and observable late phases, and direct supervision signals are ultimately needed to pull predictions back toward the true values, rather than relying on extrapolation of the fitted curve alone. The time-varying residual scale is a secondary implication: it indicates that a fixed-variance loss is inappropriate and Stage~2 must learn an input-dependent \(\sigma\).
...
\textbf{Parameter identifiability.} The third diagnostic tests whether the three q1 parameters are independently determined by the data --- if they compensate strongly for one another, individual parameter values will be hard to interpret and regressions against operating conditions will be weak. Asymptotic pairwise correlations are obtained from the trust-region Jacobian \(J\) returned by SciPy via \(\Sigma\approx(J^TJ)^{-1}\hat\sigma^2\). Across all clean fits the median values are \(\overline{\mathrm{corr}}(\log k_{1/4},\log s)\approx 0.76\), \(\overline{\mathrm{corr}}(\log k_{1/4},\log t_0)\approx 0.56\), and \(\overline{\mathrm{corr}}(\log t_0,\log s)\approx 0.21\).

These correlations confirm that the three q1 parameters are not independent physical handles but a coupled three-dimensional smoothing surface with substantial soft redundancy. This is the price paid by the differentiable sigmoid gate relative to the rigid stage-by-stage breakpoint of Hiroyasu--Arai, and it also explains why the gating parameters scale poorly with operating conditions in the regression above: the data constrains the surface as a whole, not each axis separately. The practical consequence shapes a downstream design choice: the surrogate models in Section~\ref{sec:mlp-curriculum-en} are trained to reproduce the full evaluated curve rather than the parameter triple itself, so the strong parameter coupling does not propagate into the trained network's input--output map.
...
\textbf{Overall assessment of the q1 reduced-order surrogate.} The combined evidence positions \(S(t)=\sigma((t-t_0)/s)\,k_{1/4}\,t^{1/4}\) as a numerically robust reduced-order trajectory surrogate rather than a new closed-form penetration law. The kinematic exponent \(t^{1/4}\) mirrors the two-regime structure of Hiroyasu and Arai \cite{HiroyasuArai1990} and the post-EOI extensions of Zhou et al.\ \cite{ZhouLiLaiWei2019,ZhouLiYi2021}, while the sigmoid gate replaces their hard breakpoint with a differentiable transition --- the structural prerequisite for using fitted curves as targets in a downstream neural surrogate. The q1 model is a reduced-order engineering interpolant, not a direct physical derivation from the Hiroyasu--Arai mechanism: it borrows the quarter-root temporal form as an empirically motivated compressor of noisy trajectories rather than as a mechanistic endpoint. The empirical \(k_{1/4}\) scaling matches the Hiroyasu--Arai free-jet predictions in \(\rho_a\) and \(d_n\) closely; the steeper \(\Delta P\) exponent is reported as an empirical observation rather than a confirmed regime shift. The four-parameter blend collapses to \(k_{\mathrm{sqrt}}\!\approx\!0\) in \(99.8\%\) of fits, so the data itself selects the quarter-root branch, and the central-window robust spread of \(\sim\SI{0.60}{mm}\) sits near the optical noise floor of the imaging chain.
...
Dataset~\#2 is built from individual $(t,S)$ discrete observations expanded from the cleaned wide-format CDF series table. After filtering, these observations are systematically right-censored, with censoring onset varying by operating condition: Mie scattering signal disappears at different times across conditions due to droplet breakup, spatial dilution, and local number-density reduction, so some conditions enter the censored regime before $t=\SI{0.5}{ms}$ while others remain observable considerably longer. A right-censoring point-table builder (codebase identifiers listed in Appendix~\ref{app:implementation_mapping}) automatically assigns a per-condition binary censoring label for each of the \num{592} condition groups from two statistical/optical criteria: (i)~the smoothed per-bin trajectory count falling below a fixed fraction of its within-condition peak (density-drop criterion), and (ii)~the per-bin $p_{95}$ penetration reaching $95\%$ of the estimated field-of-view cap (FOV-saturation criterion). All observations at or beyond the first bin satisfying either criterion are excluded, leaving $N_{\mathrm{unc}}=\num{506634}$ uncensored points for the downstream log-log regression.
...
\item \textbf{Re-introducing \(d_n\) as a standalone input is strictly harmful under LONO.} Adding the standardised diameter as a separate input on top of any amplitude-scaled variant increases the mean MAE by \SI{4}{}--\SI{6}{mm}: the variant adding diameter but no pressure residuals reaches \SI{12.06}{mm}, and the two variants that add both diameter and pressure residuals are the worst performers in the table (\(>\SI{14}{mm}\)), even slightly below the legacy nine-feature reference. Because \(d_n\) is already encoded in \(A\) and has only six discrete levels with severe confounding (Figure~\ref{fig:sparse-support-topology}), the redundant channel lets the network memorise nozzle identity rather than generalise across families. This is the strongest direct evidence supporting the decision to leave \(d_n\) inside \(A\) rather than re-expose it to the network.
\end{enumerate}

The production configuration (\(\alpha=0.5\), with log-pressure residuals, without standalone diameter) obtains the lowest mean LONO MAE (\SI{7.46}{mm}) and the most consistent per-fold performance of all nine configurations tested, with no single-fold result exceeding \SI{19}{mm}; the legacy nine-feature reference baseline reaches \SI{27.29}{mm} on its worst fold (Nozzle4). Even the simplest amplitude-scaled variant (\(\alpha=0.25\), no residuals, no standalone diameter) improves on the legacy baseline by \(\sim\!\SI{4.6}{mm}\) on the LONO mean, confirming that the amplitude-scaling step is the load-bearing element of the engineered-feature design. The Nozzle0 fold dominates the mean because it contributes 1{,}038 file-level sample groups --- roughly five times more than any other fold (Nozzle0 was the original first-generation nozzle series with a substantially larger experimental campaign spanning more operating-condition combinations than the later Nozzle1--Nozzle8 families) --- and is the only first-generation nozzle family with a substantially different geometric design from Nozzle1--Nozzle8, so its out-of-nozzle gap is inherently larger; even on this fold the production configuration remains within \(\SI{0.21}{mm}\) of the best amplitude-scaled variant without standalone \(d_n\).
...
The derivative penalties in the loss encode the physical expectation that spray penetration is monotonically increasing and decelerating after the transient onset --- properties that the q1 fitted targets already satisfy by construction, but which must be explicitly reinforced when the training set expands to the full noisy archive in Stage~2. The loss applied to the non-dimensional target $\hat{S}$ is
\[
\begin{aligned}
...
To refine the early-time dynamics against raw CDF penetration observations while preventing collapse in the late-time sparse regime, a knowledge distillation (KD) stage is introduced on top of the Stage-2 teacher. In population terms, Stage~3 mixes Dataset~\#1 raw observations with the Stage~2 teacher learned from Dataset~\#3. The student network inherits the teacher weights and is further trained on raw CDF penetration together with teacher guidance.
...
\]
$\mathcal{L}_{\mathrm{onset}}$ is a binary cross-entropy loss on a new onset logit output against a ramp target $\min(t/t_{\mathrm{ramp}},1)$ active only for $t\leq\SI{0.2}{ms}$ ($\lambda_{\mathrm{onset}}=0.1$); $\mathcal{L}_{\mathrm{anchor}}$ is a quadratic penalty pulling $\mu(t\approx0)$ toward zero, ramping down over the first \SI{0.15}{ms}. This anchor term is retained in the general objective and in the baseline run ($\lambda_{\mathrm{anchor}}=0.01$), but the final selected checkpoint sets $\lambda_{\mathrm{anchor}}=0$ to eliminate a systematic early-time under-prediction bias that the anchor ablation demonstrated when $\lambda_{\mathrm{anchor}}>0$; the anchor ablation results are documented in the Stage-3 training diagnostics and confirm that removing the anchor reduces near-onset mean error without degrading late-time accuracy. Here ``baseline run'' refers to the initial Stage-3 configuration before the anchor ablation was completed; the ``final selected checkpoint'' is the configuration adopted after the ablation confirmed that the anchor term caused systematic early-time under-prediction. $P_{d_1}=\operatorname{ReLU}(-\Delta\mu)^2$ enforces monotonicity; and $P_{d_2}=\operatorname{ReLU}(+\Delta^2\mu)^2$ enforces concavity for $t\geq\SI{0.5}{ms}$.
...
\]
Under a Gaussian predictive distribution with mean $\mu$ and standard deviation $\sigma$ this reduces to a closed-form expression that simultaneously rewards accuracy (small $|\mu-y|$) and calibration (neither too wide nor too narrow $\sigma$). Lower CRPS is better; a perfectly sharp and accurate point forecast has $\mathrm{CRPS}=0$. Because observations within a single trajectory are temporally correlated, the CRPS values reported here should not be interpreted as statistics over independent samples; they quantify per-point accuracy and calibration in a pointwise sense rather than distributional fit quality in an independent-observation sense.
