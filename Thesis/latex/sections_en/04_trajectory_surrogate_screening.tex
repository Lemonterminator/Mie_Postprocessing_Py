...
\subsection{Spatial right-censoring audit of the CDF penetration traces}
...
Hiroyasu and Arai divide diesel spray penetration into two stages---pre-breakup (\(\sqrt{t}\) dominant) and post-breakup (\(t^{1/4}\) dominant) \cite{HiroyasuArai1990}---and the two-branch candidate model in this section builds directly on that framework. Two adaptations are introduced: a differentiable sigmoid gate replaces the hard staged switch, yielding a smooth differentiable fitted curve; and the fit is kept free of any external physical quantities beyond the penetration observations themselves, so that each trajectory can be fitted independently. Additionally, later work on start-of-injection transients shows that short injections are difficult to cover accurately with a single rigid empirical expression over the entire trace \cite{ZhouLiYi2021}---a consideration that is especially prominent here because the present experiments use pilot diesel injection, for which the near-SOI segment is short, highly transient, and has a relatively low signal-to-noise ratio.
...
This masking process introduces a deliberate engineering trade-off, though its dominant mechanism has shifted with upstream CV pipeline improvements. In earlier processing, uneven illumination or plume-to-plume occlusion sometimes caused the CV pipeline to misidentify background noise as the spray tip, producing physically impossible jumps that were then truncated into heavily shortened series by an explicit upper difference threshold; the \(n \ge 10\) frame floor and the \(t_0 < \SI{0.8}{ms}\) plausibility gate were primarily designed to intercept these degraded trajectories. Following CV enhancement, the vast majority of candidate series now pass the full masking pipeline (per-nozzle survival rates are reported in Figure~\ref{fig:filter-survival-cdf-nozzle} below). The sequences that do get flagged tend to be \emph{longer} trajectories whose \(t_0\), RMSE, or \(\mathrm{cost\_per\_point}\) exceed the three-sigma robust threshold within their group, rather than being primarily short, noise-truncated series from CV misidentification. The masking logic therefore now functions more as a fit-quality gate than a CV-artefact cleaner.

Consequently, the filtering process introduces selection bias along two dimensions. The first is optical quality bias: the surviving dataset overwhelmingly favours well-illuminated, unobstructed, high-contrast plumes and is blind to genuine geometric asymmetries or late-stage evaporative fading. The second is injection-pattern regularity bias: q1 is structurally limited to monotonically increasing smooth penetration curves; trajectories exhibiting non-monotone behaviour from bimodal injection or spray re-acceleration have elevated within-group RMSE and are flagged by the robust z-score outlier gate. Per-nozzle flagging rates quantify the resulting coverage gap: on the CDF source, Nozzle~3 is flagged at 11.9\%, approximately twice the rate of Nozzle~4 and Nozzle~6 (both 5.9\%), which aligns with the spray re-acceleration events observed for Nozzle~3 in the Stage-3 diagnostics. Stage-1 and Stage-2 therefore inherit both biases simultaneously: their training set is skewed not only toward optically ideal plumes but also toward trajectories whose injection pattern is regular enough for the q1 monotone form to represent adequately.

Figure~\ref{fig:filter-survival-cdf-nozzle} shows the CDF penetration q1 filter survival rate per nozzle, summarising total series, cleaned series, and pass ratio. The purpose of this figure is not to argue that the filter is unbiased; it is to make the coverage of that bias explicit. After regenerating the report against the restored PC archive, the nine-nozzle campaign spans a CDF penetration survival-rate band of \(88.1\%\)--\(94.1\%\). The restored DS300/Nozzle0 archive contributes \(\num{15475}\) candidate CDF penetration series, of which \(\num{14274}\) survive the clean-fit filter (\(92.2\%\)), so it is part of the reported coverage. Because no nozzle is nearly fully rejected or nearly fully retained, Stage~2 can be interpreted as a clean-plume teacher over the restored archive, while still inheriting the location and late-stage selection bias made explicit here.

\begin{figure}[t]
\centering
\includegraphics[width=0.88\linewidth]{images/filter_survival_cdf_by_nozzle.png}
\caption{CDF penetration q1 filter survival rate regenerated from the restored PC archive. The figure reports the total candidate series count, cleaned series count, and pass ratio for all nine nozzle families, including DS300/Nozzle0.}
...
Folding $P_{\mathrm{inj}}$ and $P_{\mathrm{ch}}$ entirely into $A$, however, removes their individual explanatory power beyond what the fixed $\Delta P^{0.5}$ exponent captures. The regression also estimates a non-zero chamber-pressure coefficient at fixed $\Delta P$ (approximately $-0.21$ to $-0.27$ across the mid-injection window), indicating that $P_{\mathrm{inj}}$ and $P_{\mathrm{ch}}$ carry independent structure that cannot be fully collapsed into $\Delta P$ alone. Furthermore, the global $\Delta P^{0.5}$ exponent is a mean-field approximation; local residual nonlinearity across the condition space is left outside the network's reach when $P_{\mathrm{inj}}$ and $P_{\mathrm{ch}}$ are absent. Nozzle diameter $d_n$ is a different case: it has only six discrete levels in the dataset and suffers severe confounding in the condition matrix (Figure~\ref{fig:sparse-support-topology}), making it an unreliable continuous input even if reintroduced; $d_n$ therefore remains inside $A$, with the H--A prior carrying its effect.
...
Before any optimisation, a scaling-validation routine tests the scaling assumption on the actual dataset: each trajectory is divided by its own $A$, and a linear-regression $R^2$ of $\hat{S}(t)$ on the sparse features is computed. In the seven-feature production run the check passed with a sparse-feature $R^2$ dropping from~$0.32$ on the physical target $S$ to~$0.016$ on the scaled target $\hat{S}$, confirming that $A$ accounts for the dominant diameter and pressure variation while the network learns residual structure on the $\log P_{\mathrm{inj}}$ and $\log P_{\mathrm{ch}}$ axes. If the check fails, training aborts by default, making the physical prior falsifiable rather than asserted. The empirically calibrated exponents embedded in $A = \Delta P^{0.5}\cdot\rho_a^{-0.25}\cdot d_n^{0.5}$ carry an empirically checkable prediction that the training protocol confirms or rejects before any weights are updated.
...
\]
where $\mathcal{L}_{\mathrm{raw}}$ is the Gaussian NLL on physical $(\mu,\sigma^2)$ against raw CDF penetration targets weighted by $w_{\mathrm{raw}}$; $\mathcal{L}_{\mathrm{KD}}$ is the forward KL divergence $D_{\mathrm{KL}}(p_{\mathrm{teacher}}\|p_{\mathrm{student}})$ in physical space weighted by $w_{\mathrm{kd}}$ and a teacher confidence factor
\[
...
\]
This factor reduces the effective KD weight in proportion to how far the teacher's uncertainty exceeds the reference scale: when $\sigma_{\mathrm{teacher}}\leq\sigma_{\mathrm{ref}}$ the teacher is confident and $w_{\mathrm{conf}}=1$; when $\sigma_{\mathrm{teacher}}\gg\sigma_{\mathrm{ref}}$, $\sigma_{\mathrm{teacher}}(t)$ reflects the spread of q1-fitted curve extrapolations within the same operating condition at that time point rather than verified physical extrapolation uncertainty; the effective KD weight is therefore reduced to a floor of $0.25$, preventing the student from being dominated by the accumulated intra-condition scatter in the censored late-time tail.
...
These Stage~3 loss values are not a direct improvement/degradation comparison against the Stage~2 scaled NLL. Stage~2 optimises fitted q1 targets on the filtered trajectory table, whereas Stage~3 mixes raw CDF penetration likelihood, teacher--student KL, onset supervision, and shape penalties on a different raw/KD-regime population. The reproducibility pipeline therefore exports each stage metric with an explicit target-population label rather than placing all NLL-like numbers on a single leaderboard.
...
It uses the ``a-only'' five-feature set (without the $\log P_{\mathrm{inj}}$ and $\log P_{\mathrm{ch}}$ pressure log-ratios used by the production MLP),
\[
[\,t_{\mathrm{norm}},\;\theta_{\mathrm{tilt},z},\;n_{\mathrm{plumes},z},\;t_{i,z},\;P_{\mathrm{cb},z}\,],
\]
...
Thus, the GP and MLP share the same target scaling ($\hat{S}=S/A$), but the GP uses five input features compared with the seven used by the production MLP; the difference is that the GP expresses the conditional function through kernels and inducing points, whereas the MLP expresses the same task through a staged neural network and distillation loss.
...
\]
with centre $\mathbf{c}(t)=\mathbf{x}_{\mathrm{nozzle}}+\mathbf{R}(\theta_{\mathrm{tilt}})\,(\mu(t),0)^{\top}$, is evaluated on a regular canvas grid (default \SI{0.25}{mm/px} over a $125\times\SI{200}{mm}$ window); a sanity pass confirms $\iint p\,\mathrm{d}A=1$ to four decimal places per frame. The student's scaled outputs are denormalised to physical space via the empirically calibrated amplitude $A=\Delta P^{0.5}\cdot\rho_a^{-0.25}\cdot d_n^{0.5}$ introduced for the engineered-feature architecture above, so that $\mu$ and $\sigma_{\parallel}$ enter the PDF in millimetres.
...
\]
which omits the higher-order connecting-rod term (the $r/l\to 0$ limit). Phase $\varphi$ places the reference event at $t=0$. The default prototype setting of $\mathrm{RPM}=250$, $s/2=\SI{55}{mm}$, $h_{0}=\SI{75}{mm}$, and $\varphi=0$ corresponds to a short injection window close to top dead centre, over which the crown moves by less than \SI{1}{mm}; in this regime the connecting-rod correction is negligible.
