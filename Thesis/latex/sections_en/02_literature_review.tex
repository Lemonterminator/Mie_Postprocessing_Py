...
This chapter uses the literature review as a design rationale rather than as an early repetition of the implementation details. It asks how existing work supports the technical choices made in this thesis: Mie-based geometric observables, short-pulse penetration behaviour, multihole plume registration, right-censored targets, uncertainty-aware surrogate modelling, and probabilistic impingement screening. It also identifies the parts of the problem that are not covered by any single strand of the literature. The purpose is to make the chain from prior knowledge to engineering design decisions explicit before the workflow itself is described.

\subsection{Introduction to Methodological Terminology for Interdisciplinary Readers}\label{sec:methods-terminology}

The implementation in this thesis spans four technical domains:
computer vision, numerical optimisation, GPU parallel computing, and deep learning.
These four tools are connected in sequence:
image processing extracts penetration observables from videos;
robust optimisation compresses noisy trajectories into stable fitting targets;
...
Performing this transformation around the calibrated nozzle centre
...
When the data contain a small number of outliers
(for example, frames corrupted by optical noise or minor quantization artefacts),
...
This matches the engineering intuition of treating anomalous readings cautiously, and has a firm basis in robust statistics \cite{Huber1964,NocedalWright2006}.
...
The engineering significance of GPU acceleration is that it turns terabyte-scale batch processing from a task that may take days into one that can finish within hours and support iterative refinement \cite{NickollsBuckGarlandSkadron2008}.
...
Modern neural-network training also depends on regularisation and optimisation choices.
Dropout randomly masks part of the neurons during training, forcing the network to learn redundant representations and reducing overfitting when operating-condition support is sparse \cite{Srivastava2014Dropout}.
Decoupled weight decay, as used in AdamW, separates the decay term from the adaptive gradient update and can improve generalisation \cite{LoshchilovHutter2019AdamW}.
Smooth gated activations such as SiLU/Swish preserve smooth derivative propagation, which is important for the derivative-based physical penalties introduced later \cite{Ramachandran2017Swish}.
The parameters are obtained through training:
the prediction error is evaluated over many samples, and the weights are iteratively updated by gradient descent until the overall error converges.
...
which is equivalent to providing an uncertainty band that varies with conditions
for each operating point,
rather than using the same fixed error band for all conditions \cite{KendallGal2017}.
...
---analogous to an experienced engineer providing a judgement baseline
when samples are insufficient \cite{HintonVinyalsDean2015}.
...
The appeal of this kind of model is practical: it gives a low-order, interpretable law for the gross penetration history.
Its limitation becomes visible when the injection event is short.
...
Since the operators used here are standard image-processing primitives \cite{GonzalezWoodsDIP,Szeliski2010Vision}, the CUDA implementation is not merely an implementation preference:
it turns the processing chain into an instrument that can be rerun as the research choices evolve, rather than a one-pass extraction script.
...
The data choice is not merely historical; it is jointly determined by the required observables and by hardware constraints.
...
This thesis uses forward KL, with the student distribution approximating the teacher distribution, because it is consistent with the negative-log-likelihood objective used in the earlier stages and tends to cover the teacher distribution in sparse late-time regions.
...
The important point is not the number of learning stages, but the way the three elements are tied together:
censoring-aware target construction, heteroscedastic regression for input-dependent uncertainty, and teacher-guided bias correction.
All three serve the same purpose:
preventing the model from learning observation truncation as physical saturation.

\subsection{Shape constraints and physics-informed neural networks}
...
\subsection{Evaluation of Probabilistic Predictions: Proper Scoring Rules and Model Calibration}
...
Physics-informed learning provides a general mechanism for imposing derivative-based constraints through automatic differentiation.
...
and teacher distillation to avoid learning observation truncation as physical saturation;
(5) sparse operating regions require derivative-based soft constraints to suppress non-physical trajectory shapes; and
(6) the predicted conditional Gaussian penetration must be mapped onto simplified engine-geometry masks
to form a probabilistic risk proxy suitable for early design screening.
The contribution of this thesis is not that it touches these issues separately,
but that it treats them as design constraints of one workflow.
The following chapters test whether that integrated workflow is sufficient for the intended engineering design-screening task.
