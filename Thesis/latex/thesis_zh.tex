...
在来自 9 个喷嘴试验批次、未删失评估协议下的 540,367 个评估点上，最终多层感知机（MLP）模型在五个随机种子下的平均均方根误差（RMSE）为 4.19 mm、平均绝对误差（MAE）为 2.98 mm，并保持 88.8\%/99.0\% 的保守一倍/两倍标准差不确定性覆盖率，相对校准后的 Hiroyasu--Arai 与 Naber--Siebers 基线，RMSE 降低超过 54\%。相对于受控的 pooled 基线，后训练 FiLM 适配改善了概率校准评分，而 RMSE 未出现统计显著变化；该证据仅限于额外开展的已知喷嘴族适配实验。作为替代主干的稀疏变分高斯过程（SVGP）点精度更优（分布内 4.09 mm；留一喷嘴测试中 5.95 mm 对 7.00 mm），但不确定性覆盖率明显不及前者保守；二者应被理解为同一删失感知协议下精度与覆盖率之间不同的权衡。最后，本文通过简化几何投影，将预测贯穿距离分布映射为概率性撞壁筛查评分。该工作流定位为测量域内的早期工程筛查工具，先于——而非替代——计算流体力学（CFD）仿真与发动机试验。}
