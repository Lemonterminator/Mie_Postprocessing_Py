...
\paragraph{神经网络与深度学习。}
本文使用的多层感知机（MLP）由若干线性层和非线性激活函数组成，以工况特征和时间为输入，输出穿深预测。Dropout 在训练中随机屏蔽部分神经元，降低稀疏工况支持下的过拟合风险 \cite{Srivastava2014Dropout}。AdamW 将权重衰减从自适应梯度更新中解耦，有助于改善泛化 \cite{LoshchilovHutter2019AdamW}。SiLU/Swish 等平滑激活函数使导数传递更连续，便于使用物理导数惩罚 \cite{Ramachandran2017Swish}。均方误差（MSE）对应条件均值回归；异方差高斯负对数似然（NLL）让网络同时输出条件均值和条件方差，为不同工况提供不同宽度的误差带 \cite{KendallGal2017}。知识蒸馏先训练一个在干净数据上表现稳定的教师模型，再用其软输出引导学生模型在标签稀疏或不可靠区域学习 \cite{HintonVinyalsDean2015}。
