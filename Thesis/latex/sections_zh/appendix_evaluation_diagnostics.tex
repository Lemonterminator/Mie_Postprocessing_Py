...
\bottomrule
\end{tabular}
\caption{受控链按喷油器族的切片（\texttt{evalrun\_\allowbreak{}20260713\_\allowbreak{}123732}，QC-gated 未删失 CDF 表，单位 mm）。pooled 基线在多数族（参考件）上近乎无偏，把系统性高估留给整修族；residual 的偏差修复几乎全部发生在整修族（\(+1.223 \to +0.253\)；族内配对 \(\Delta\)RMSE \(-0.072\)，显著），代价是参考件族上轻微但显著的退化（\(+0.063\)），整体 RMSE 持平是两族相消的净值。FiLM 的均值修正在两族上几乎相同（\(-0.293/-0.290\)），其收益集中在概率指标。}
