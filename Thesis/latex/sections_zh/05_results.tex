...
第~4 章将穿深 surrogate 定义为一个嵌入光学归档到筛查工作流中的、具备删失感知的低维概率回归问题。因此，本章结果不再组织成单一的排行榜，而是一组相互关联的评估视图。full-clean CDF 协议在 Dataset~\#1 的所有已清洗留出 CDF 点上测试选定的域内逐点 surrogate。uncensored CDF 表是 Dataset~\#2 中主要的删失控制逐点检查。P50-observed 和 q1-extrapolated 表是工况级检查，用于分离窗口内的代表性行为与外推延续行为。留一喷嘴族（LONO）协议测试对未见喷嘴族的转移能力，最后的消融部分则对使 MLP 能够用于下游筛查工作流的各项设计选择进行归因。
...
在刷新后的全净化 CDF 评估协议下，采用部署蒸馏损失形式（均值 MSE 与对数方差 MSE 的混合，方差权重 \(\lambda_\sigma=5.0\)）在五个种子（\(\{7,17,42,99,2024\}\)）上训练的选定无锚点阶段三 MLP 筛查 surrogate，在每个种子的 24{,}316 条轨迹、692{,}942 个留出时间点上达到整体 RMSE~\(\SI{4.735}{mm}\pm\SI{0.037}{mm}\)、MAE~\(\SI{3.293}{mm}\pm\SI{0.039}{mm}\) 以及微小的正向偏差~\(\SI{+0.626}{mm}\pm\SI{0.112}{mm}\)。这些清洗后的 CDF 轨迹中有 9,630 条为右删失轨迹，对应 SCR 为 39.6\%；把最后一个可见点当作物理终点会引入实质性的协议误差，而不是单纯的记录细节。图~\ref{fig:pred-vs-actual-best} 表明，在整个测量范围内，留出集预测基本贴近恒等线，且随着穿深增大，散点逐步变宽。这种变宽与异方差设计是一致的：模型并没有强行对所有时刻和工况使用同一个固定误差条。

不确定度校准在工程上同样有意义。全清洗协议下的最终 5 种子平均 1$\sigma$ 与 2$\sigma$ 覆盖率分别为 86.7\% 和 98.5\%，明显比名义高斯值 68.3\% 和 95.4\% 更保守。图~\ref{fig:stage3-calibration-coverage} 进一步把这一点展开为按预测标准差分箱的可靠性图和覆盖率曲线，而不只比较平均标准差与平均误差。对于早期设计筛查而言，这种保守偏置有用，但不能被过度解读为经过认证的假阴性安全保证：它使模型不太容易低估晚期时间或弱观测区间下的穿深不确定度，但有害液态撞壁仍需要发动机代表性验证。
...
选定的域内无锚点筛查 surrogate，具有更好的早期保真度和保守的晚期行为；最终核心指标见下方的外部清洗协议。 \\
\bottomrule
\end{tabular}
...
上述留出集结果反映了阶段三在最初归档训练与评估单一切分下的表现。为了检验结论是否依赖该单一切分，本文进一步引入一个外部清洗评估协议。刷新后的全净化 CDF 协议包含 692{,}942 个预测点和 24{,}316 条轨迹，所有模型都在同一目标集合上进行评估。表~\ref{tab:external-clean-headline} 汇总了物理基线、选定的 MLP 变体以及阶段三稀疏异方差高斯过程（SVGP）替代主干：校准后的 Hiroyasu--Arai 经验关联式、带延迟项的 Naber--Siebers 关联式、选定的多种子 MLP 筛查 surrogate、面向外推正则化的诊断 KD 变体（仅 $\mu$-锚点、不确定区间使用原始 NLL 监督），以及阶段三 SVGP。物理基线的拟合协议见第~\ref{sec:baseline-fitting-protocol-en} 节，SVGP 的构造见第~\ref{sec:svgp-alternative-backbone-en} 节。在选定无锚点配置下五次重复训练的平均 RMSE 为 \SI{4.735}{mm}、MAE 为 \SI{3.293}{mm}、P95 绝对误差为 \SI{9.167}{mm}，并给出 0.867/0.985 的保守 1$\sigma$/2$\sigma$ 覆盖率。相对 Hiroyasu--Arai 与 Naber--Siebers，RMSE 分别降低 58.0\% 和 55.1\%。SVGP 在点预测 RMSE/MAE 上占优（RMSE \SI{4.628}{mm}），并且后文显示其在 LONO 下转移更强；因此当前的结论并不是 MLP 是普遍更优的回归器，而是选定 MLP 是嵌入低成本、可审计运行工作流中的保守域内筛查 surrogate。随后，预测分布被投影到图~\ref{fig:impingement-preview-zh}--\ref{fig:impingement-summary-zh} 所示的简化二维发动机几何模型上，产生面向排序的几何壁面接近筛查指标，而非确定性的通过/不通过阈值，也不是真实有害液态撞壁概率。
...
图~\ref{fig:calibration-current} 展示了在同一 full-clean 诊断协议中加入 Stage~3 SVGP 后的可靠性图与 CRPS--sharpness 平面。三阶段 MLP（anchor-off）的 1$\sigma$/2$\sigma$ 覆盖率（五种子均值 0.867/0.985）高于名义高斯值，因此在外部清洗集合上是有意保守的。表~\ref{tab:external-clean-headline} 中刷新后的 Stage~3 对比显示，SVGP 现在是最强的点预测器（RMSE \SI{4.628}{mm}，1$\sigma$/2$\sigma$ 覆盖率 0.703/0.939），并在刷新后的校准诊断中获得了最低的 CRPS；而选定 MLP 仍是更为保守的域内筛查代理模型（RMSE \SI{4.735}{mm}，覆盖率远高于名义值）。因此，这两个模型应被理解为不同的精度--覆盖率权衡，而非单赢结果：SVGP 更接近名义覆盖率且点精度更强，而 MLP 为下游筛查保留了更宽的不确定度裕度和更低开销的工作流。
...
\bottomrule
\end{tabular}
\caption{保守 uncensored CDF 表上的域内爬升。RMSE、MAE、CRPS 与 P50 RMSE 单位为 mm；ECE 为期望校准误差。第一组为表~\ref{tab:external-clean-headline} 的参考模型，第二组为残差扩展。“QC” 列为 QC 门控数据重训后的 RMSE（第~\ref{sec:residual-robustness-zh} 小节），作为数据敏感性列。粗体为新最优。}
