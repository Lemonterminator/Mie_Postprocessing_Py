...
\centering
\includegraphics[width=\linewidth]{../images/GUI_calibration_interface.png}\\
（a）选点
\end{minipage}
\hfill
\begin{minipage}{0.47\linewidth}
\centering
\includegraphics[width=\linewidth]{../images/GUI_calibration_result.png}\\
（b）标定叠加结果
\end{minipage}
...
尾部截断之后，构建有效帧掩码，并用 \texttt{remove\_short\_true\_runs(..., min\_len=5)} 去掉孤立的短区段。
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\linewidth]{../images/raw_cv_signal_loss.png}
\vspace{3pt}
\includegraphics[width=0.92\linewidth]{../images/raw_cv_fov_censoring.png}
\caption{三种穿深定义的原始逐录像 CV 输出——累积分布前沿（\texttt{penetration\_cdf}）、最大索引轴向（\texttt{penetration\_bw\_x}）与最外层边界极坐标（\texttt{penetration\_bw\_polar}）——每个面板中叠加全部十个羽流（顶视多孔录像；已去除批次标识）。上图（信号丢失）：CDF 前沿保持稳定平台，而两种二值度量随后期 Mie 信号褪去而断裂、塌缩。下图（视场删失）：三种度量在共同的 $\approx 92$\,mm 上限处趋平，对应喷雾越过相机窗口边界，而非到达穿深的物理极限。}
...
\caption{用于检测液压延迟和喷嘴关闭事件的近喷嘴时序信号。鲁棒归一化后的块强度（蓝色）在液压延迟处（红色竖实线）升过高阈值（红色虚线），在喷嘴关闭处（黑色竖实线）降至低阈值（橙色虚线）以下；检测到的有效喷射区间以绿色高亮显示。单孔录像，仅用于说明；相同检测算法适用于全部喷嘴。}
