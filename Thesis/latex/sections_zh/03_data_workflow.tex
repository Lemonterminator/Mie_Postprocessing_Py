...
原始输入（高速 Mie 视频、标定配置和试验元数据）先经过 CUDA 加速的图像处理，生成一维特征和边界点；在此基础上，再得到基于二值（BW-based）和基于 CDF 的穿深轨迹以及其他几何度量。
这些观测量通过无参数曲线拟合扩展为合成轨迹，并用于驱动三阶段神经网络训练：阶段~1（MSE）、阶段~2（NLL）和阶段~3（Raw + KD）。
...
这条 CUDA 流程并不是逐帧 MATLAB 脚本的简单移植，而是一个端到端的归档处理调度器，围绕三项工程防护展开。
视频加载采用深度为 1 的生产者--消费者预取：当录制 \(N\) 在 GPU 上处理时，后台 worker 从磁盘读取录制 \(N+1\)，使 GPU 计算与磁盘 I/O 重叠而非串行。
主机到设备的传输和 CuPy 数组构造刻意保留在主线程中完成，因为 CuPy 的 CUDA 上下文与线程绑定；这样可以让 kernel 调度、同步和内存池状态都由同一个线程统一管理，而无需锁开销。
输出写盘由一个长期存在的三 worker I/O 池处理：metrics CSV、metadata JSON、边界归档以及可选的预览视频文件，会在 GPU 阶段完成后立即作为 future 提交，使下一条记录的 GPU 处理与上一条记录的磁盘写入重叠。
...
\subsubsection{CUDA 后端上的秩-1可分离 Sobel 高通}
...
当前实现首先对每个羽流的 \((t,y,x)\) 高通条带体做基于百分位数的最小--最大归一化，把 \(q_{\min}\) 和 \(q_{\max}\) 百分位映射到 0 和 1，并截断范围外的值。
...
可复用库函数在交互式测试时保留一个更低的默认值；本文所有定量结果都使用批处理脚本写入归档的 \(q=0.995\)。
...
本文并没有把喷雾边界提取表述成像素级监督分割任务，因此也没有把 IoU 或 F1 score 这类依赖人工标注真值掩码的指标作为主要评估标准。
对于高速 Mie 图像，液滴散射通常在亮前景和暗背景之间制造出直接的局部对比；与 Schlieren 图像里常见的背景折射率纹理和大尺度灰度变化相比，这种成像特征更适合用梯度增强和形态学清理来提取一个可操作的边界。
喷雾边界本身也会受到雾化、破碎和灰度过渡影响，因此并不存在唯一的像素级真值。
如果要建立 IoU 或 F1 基准，就需要在大规模时间序列上逐像素人工标注，而这些标注本身仍然会带有边界判断的主观性。
因此，本文通过 Mie 散射成像机理、叠加检查、时间连续性以及多个几何代理量之间的一致性来评估提取边界的可靠性。
这种评估方式与本文目标一致：稳定提取穿深、锥角和边界包络观测量，而不是训练或比较一个通用语义分割器。
支持这些观测量可靠性的定量证据——CDF 与二值穿深估计之间的跨度量一致性、时间连续性检查，以及留一喷嘴族诊断——将在第~5 章给出。
