...
\smallskip\noindent\textbf{本章贡献.} 具体而言，本章为从图像到观测量的处理流水线贡献了五个可复用的方法学要素：(1)~对数比（同态）前景估计，将眩光与非均匀照明视为乘性效应，并在对数域中以加性偏置将其去除（第~\ref{subsec:background-normalization}~节）；(2)~秩一可分离的 Sobel 边缘增强，整段保持在 GPU 上、以两次一维卷积完成（第~\ref{subsec:edge-enhancement}~节）；(3)~基于 FFT 谐波相位的对齐，把已知的多孔数转化为逐录像自动的羽流配准规则，而非逐录像手工设定参考角（第~\ref{subsec:plume-registration}~节）；(4)~基于累积分布（CDF）的穿深前沿，对破碎的前缘像素稳健，同时与标准柴油喷雾穿深惯例保持一致（第~\ref{subsec:penetration-measurement}~节）；(5)~四泳道重叠的 GPU 流水线及可恢复检查点合约，可在单块工作站 GPU 上端到端重新处理多 TB 归档（第~\ref{subsec:concurrency-architecture}~节）。
...
\]
并通过标准线性求解器（例如 \texttt{np.linalg.lstsq}）以最小二乘形式求解。恢复得到的参数为：
\[
...
只标定一次、并为所有帧复用同一个中心，可以避免下游羽流角度分析中的帧间抖动。这种一次性标定刻意保留为用户可见的操作，而不是全自动：操作者可以放大、选择代表性帧，并用一个物理上有意义的喷嘴中心来锚定整个批处理 \cite{Pratt1987CircleFit,Taubin1991CircleFit}.
...
对于每段记录，处理链需要回答五个实际测量问题：喷嘴中心在哪里、每条羽流方向在哪里、如何把每条羽流表示到共同的局部坐标系中、液相前沿已经穿透多远，以及哪些边界描述符可以被一致提取。
以下小节按这一顺序展开。
...
在第二段录像（下图）中，三种度量却一起攀升，并在共同的 $\approx 92$\,mm 上限处趋平——这并非穿深的物理终点，而是喷雾越过了有限的相机视场边界，是一种对观测的结构性右删失。
...
针对上述基于 CDF 的可观测量的定量下游证据——尤其是留一喷嘴（LONO）迁移诊断以及删失（此处右删失指喷雾贯穿前沿在喷射结束前已到达相机视场边界，使观测轨迹被截断，而非物理饱和）可控的预测行为——在第~5~章呈现。
