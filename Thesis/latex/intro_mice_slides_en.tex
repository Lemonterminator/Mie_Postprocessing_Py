...
\begin{enumerate}
  ...
  \item \gaptext{Control problem}: pilot spray penetration governs ignition-kernel placement, main-fuel mixing, and combustion efficiency.
  \item \gaptext{Boundary risk}: if that spatial control fails, the same spray may create impingement, lubricant dilution, and deteriorated combustion.
...
\begin{itemize}
  ...
  \item Narrowing step: pilot spray is not only an ignition aid; its spatial development is a control variable for ignition-kernel placement and main-fuel combustion.
...
\vspace{0.8em}
\begin{block}{Thesis argument}
Pilot-spray spatial development and its wall-risk boundary are commonly assessed through engine tests, CFD and spray-combustion simulations, empirical penetration models, and optical spray experiments. Diesel-spray research already has a foundation in high-speed imaging, empirical correlations, phenomenological models, and CFD \src{Linne2013SprayImaging; HiroyasuArai1990; NaberSiebers1996; liu2020sprayreview}.
\end{block}
...
What is missing is a fast and repeatable method for predicting the time-evolving non-reacting liquid envelope of pilot diesel spray, so that pilot-injection control and later wall-risk analysis both have a defensible geometric input.
\end{block}
...
Convert a large Mie-scattering spray-video archive into an early-stage engineering screening layer between optical experiment archives and high-cost validation.
\end{block}

\vspace{0.3em}
\begin{enumerate}
  \item Extract plume-resolved geometric observables.
  \item Construct censoring-aware penetration targets.
  \item Train an uncertainty-aware surrogate.
  \item Connect the output to pilot-injection control assessment and a probabilistic wall-impingement screening proxy.
\end{enumerate}
\end{frame}
...
\vspace{0.5em}
\begin{enumerate}
  \item OSCC room-temperature, non-reacting, top-view high-speed Mie recordings.
  \item Manual nozzle calibration: centre, radius, angular reference, and pixel scale.
  \item CUDA image chain: robust preprocessing, Sobel, polar/FFT registration, and plume-strip extraction.
  \item Trajectory chain: CDF penetration, boundary metrics, cleaning, reduced-order fitting, and censoring treatment.
  \item Model chain: Stage~1 MSE, Stage~2 heteroscedastic NLL, and Stage~3 raw-CDF/KD refinement.
\end{enumerate}
\end{frame}
...
\vspace{0.5em}
\begin{block}{Compact result}
Under the external-cleaning protocol, the transition-region-scaled anchor-off Stage~3 surrogate reaches a four-run mean RMSE of~\SI{6.094}{mm} on 795{,}561 prediction points from 33{,}845 trajectories, with conservative 1$\sigma$/2$\sigma$ coverage of 0.783/0.982.
\end{block}

\vspace{0.4em}
\begin{itemize}
  \item The earlier original held-out split result, RMSE~\SI{6.68}{mm} on 21{,}470 trajectories, is retained as a legacy split result rather than the headline claim.
  \item The output supports pilot-control comparison and rankable probabilistic screening, not a binary pass/fail verdict.
  \item Results and interpretation are left to the Results and Discussion chapters.
\end{itemize}
\end{frame}
...
\vspace{0.6em}
\begin{tabular}{L{0.31\linewidth}L{0.56\linewidth}}
\toprule
Chapter & Function \\
\midrule
...
Experimental data and image-processing workflow & Covers OSCC data, manual calibration, CUDA preprocessing, registration, strip extraction, and CDF/boundary metrics. \\
Trajectory targets, surrogate modelling, and probabilistic screening & Covers cleaning, reduced-order fitting, feature design, Stage~1--3 learning, and the screening proxy. \\
Results & Reports accuracy, campaign variation, ablation evidence, and prototype screening output. \\
Discussion & Interprets Stage~3 credibility, validated claims, and scope boundaries. \\
Conclusions & Summarizes validated outcomes, prototype limits, and next steps. \\
\bottomrule
\end{tabular}
\end{frame}
...
Move 1-2 & Research relevance & Multi-fuel engines rely on pilot fuel, and spray spatial development controls ignition-kernel placement and main-fuel combustion. \\
