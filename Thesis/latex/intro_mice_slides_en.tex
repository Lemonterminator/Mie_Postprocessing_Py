...
\begin{enumerate}
  ...
  \item Connect the output to a probabilistic wall-impingement screening proxy.
\end{enumerate}
\end{frame}
...
The final Stage~3 surrogate reaches RMSE~\SI{6.60}{mm} on roughly 15{,}000 trajectories from 9 nozzle campaigns, about 6.88\% of the measured penetration range, with near-Gaussian and slightly conservative coverage: 73.56\% / 96.93\%.
\end{block}

\vspace{0.4em}
\begin{itemize}
  \item The output is a rankable probabilistic screening proxy, not a binary pass/fail verdict.
