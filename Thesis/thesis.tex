...
The final model is evaluated on roughly 15{,}000 trajectories drawn from 9 nozzle campaigns. 
It reaches an overall RMSE of \SI{6.60}{mm}, equivalent to 6.88\% of the measured penetration range, 
and retains uncertainty coverage close to the Gaussian expectation while remaining slightly conservative 
(73.56\% at 1$\sigma$ and 96.93\% at 2$\sigma$). 
...
Error is not uniform across campaign folders. Figure~\ref{fig:per-folder-rmse-best} shows the mean and median per-trajectory RMSE for the final model across the evaluated subfolders. The easiest folders remain in the low-\SI{3}{mm} range, while the hardest approaches \SI{9}{mm}. The Nozzle0 folder sits in the middle of this ranking rather than at either extreme, which is consistent with the later ablation evidence: Nozzle0 benefits materially from the final raw/KD balance, but it is not the sole source of global error.
...
The ablation compared four configurations against the final tuned setting:
(i)~the final tuned configuration ($w_{\mathrm{kd}}=0.25$ in \texttt{raw\_reliable} bins, onset and curvature penalties active);
(ii)~a more aggressive raw-supervision variant with no KD blending in reliable bins ($w_{\mathrm{kd}}=0$);
(iii)~the final weights but with the onset loss disabled ($\lambda_{\mathrm{onset}}=0$); and
(iv)~the final weights but with the concavity penalty disabled ($\lambda_{d_2}=0$).
Figure~\ref{fig:ablation-best} compares these settings across overall RMSE, residual bias, and Nozzle0 subset performance. Two points matter most. First, the final tuned setting is the only variant that simultaneously minimises overall RMSE and residual bias. Second, the gain is not confined to one dataset slice: the Nozzle0 panel also improves relative to the baseline. This supports the interpretation that the final model is not merely overfitting a global scalar metric; it is improving the raw/KD balance in the regime where early-time observations are trustworthy while keeping the teacher active where late-time support collapses.
...
\caption{Ablation summary for Stage~3 tuning. The final tuned setting gives the best joint trade-off across overall RMSE, residual bias, and Nozzle0 response.}
