...
It is worth noting that the dataset used in this thesis was not chosen by optimizing over available diagnostic modalities but was dictated by the experimental source: the data come from top-view Mie-scattering experiments on a multihole nozzle, even though the same laboratory also has Schlieren, diffuse back-illumination (DBI), and chemiluminescence imaging capability. In practice, each diagnostic has its own physical sensitivity: Schlieren responds to density gradients, DBI measures path-integrated optical thickness, and chemiluminescence indicates the reaction zone. Mie scattering is sensitive to forward and side scattering from liquid droplets, and thus provides high contrast at the liquid-phase boundary, making it well suited for extracting penetration depth and cone angle \cite{Linne2013SprayImaging}.

Mie imaging also exhibits a characteristic multiple-scattering effect: in the dense liquid-core region near the nozzle, multiple scattering and internal shadowing elevate the local intensity while producing little sharp directional structure, resulting in a diffuse halo rather than a clean plume boundary \cite{BohrenHuffman2004,Linne2013SprayImaging}. The present approach suppresses this effect through Sobel high-pass filtering: the halo contributes to the slowly varying background intensity field but contributes very little to the local gradient field. Gradient-domain processing therefore acts as a first-order rejection of the multiple-scattering halo without requiring explicit physical modeling of the scattering regime. For the optical path, this thesis adopts a line-of-sight integration simplification: the recorded Mie signal is treated as a path-integrated quantity from a single focal plane. Although the true plume geometry has an umbrella angle and is not strictly planar, this assumption is appropriate for the present goal of macro liquid-envelope characterization rather than three-dimensional liquid-phase reconstruction.
...
The breakdown of quasi-steady empirical models is not caused solely by fluid-dynamic staging; it is also compounded by the dynamic behavior of the upstream injector hardware. In a common-rail injection system, the needle-valve opening and closing events are finite-time mechanical transients: before the needle is fully lifted, the effective flow area through the nozzle varies with time, directly affecting the instantaneous injection rate and the near-nozzle flow field \cite{StumppRicco1996,HiroyasuArai1990}. Under high injection pressures, cavitation inside the nozzle can further alter the internal jet structure and affect atomization characteristics in ways that classical penetration correlations do not explicitly capture \cite{BohrenHuffman2004}. Both effects are especially pronounced for short pilot injections, where the needle-opening and closing transients can occupy a substantial fraction of the total injection event, and the quasi-steady assumption holds for only a narrow middle window, if at all. Data-driven models that are not committed to a specific model-form therefore have a practical advantage over rigid power-law correlations when fitting short-injection traces: they do not require the transient regimes to be identified and parameterized in advance, and they can accommodate the combined effect of multiple overlapping physical mechanisms without constructing a piecewise rule for each.
...
The finite field of view in optical spray imaging creates a statistical problem as well as a geometric one. When the true late-time penetration extends beyond the visible window, the recorded trajectory is not simply noisy; it is right-censored by the observation limit. In statistics, this is closely related to the broader class of limited or censored dependent-variable problems, where the observed value is only a truncated representation of the latent quantity of interest \cite{Tobin1958,BuckleyJames1979}. Although both references originate in an econometric context, the core statistical structure --- a dependent variable that can only be observed below a threshold, with information beyond that threshold suppressed --- is formally identical to the situation in optical spray imaging, where the true penetration exceeds the field-of-view boundary and the apparent terminal value is an observation limit rather than a physical one.
...
The method-dependence of spray measurements is also systematically documented in the Engine Combustion Network (ECN) benchmark work. By testing nominally identical injectors (Spray~A and Spray~D) across multiple institutions, ECN found measurable inter-facility variation in liquid-penetration values and explicitly concluded that optically measured liquid boundaries should be understood as quantities defined by a particular measurement protocol, not as absolute ground-truth representations of the liquid phase \cite{ecn_liquid_penetration,ecn_liquid_penetration_accessory,MeijerECN2012}. The experimental context in this thesis differs from ECN Spray~A (high-temperature, high-pressure evaporating conditions) and Spray~D (multihole nozzle) — here the data come from a room-temperature pressurized chamber with a multihole marine nozzle and without fuel evaporation — but the ECN methodological principle applies equally: the numerical values of reported penetration and boundary metrics depend on the post-processing rules, and that dependency must be stated explicitly when results are interpreted.

\subsection{Why deterministic, uncertainty-aware, and teacher-guided learning stages are all useful}

Data-driven surrogate models have demonstrated clear viability for predicting spray macroscopic characteristics in recent literature. Neural-network approaches have been used to map operating-condition parameters directly to spray tip penetration and cone angle without requiring a full CFD computation as an intermediate step, with several studies reporting prediction accuracy approaching 95\% \cite{VazKarathanasis2026,liu2020sprayreview}. These results confirm that neural networks are a practically feasible modeling layer for spray macroscopic properties. However, those studies typically assume complete, fully observable penetration trajectories as training labels and do not model inter-condition or inter-cycle uncertainty explicitly. The constraints in the present thesis are more restrictive on all three counts: the short pilot-injection traces are dominated by SOI and EOI transients, the limited field of view produces a large fraction of right-censored trajectories, and the plume-to-plume and shot-to-shot variance is itself engineering-relevant information rather than irreducible noise. The learning framework here therefore extends general data-driven spray surrogates with three design elements that address those specific constraints: censoring-aware target construction, heteroscedastic uncertainty quantification, and a distillation-based refinement stage.
...
The Gaussian output assumption carries an additional computational advantage in the distillation stage. When both teacher and student outputs are represented as univariate Gaussian distributions, the Kullback--Leibler divergence between them has a closed analytical form:
\[
D_{\mathrm{KL}}\!\left(\mathcal{N}(\mu_1,\sigma_1^2)\,\|\,\mathcal{N}(\mu_2,\sigma_2^2)\right)
=\log\frac{\sigma_2}{\sigma_1}+\frac{\sigma_1^2+(\mu_1-\mu_2)^2}{2\sigma_2^2}-\frac{1}{2},
\]
which allows the distillation loss to be computed exactly, without Monte Carlo sampling, and therefore without introducing additional gradient variance during training. Regarding the direction of the divergence: forward KL (student approximating teacher) is mean-seeking and distributes mass broadly, whereas reverse KL is mode-seeking and concentrates on the teacher's main peak with less influence from sparse outlier trajectories. In practice both directions give comparable results in this application, and forward KL was chosen for consistency with the negative log-likelihood training used in Stages 1 and 2 \cite{BishopPRML2006,KendallGal2017}. Knowledge distillation for regression under data scarcity has been shown to reduce student prediction error substantially in regimes where the original training set is incomplete, because the teacher provides stable soft targets in regions where direct labels are absent \cite{KangKang2024KDRegression}.
...
Taken together, the literature reviewed in this section establishes the following foundations for the present work: Mie scattering theory maps optical intensity to geometric observables in a defensible way; computer vision and signal processing theory supports each step of the image-processing chain; diesel spray physics defines the physical meaning and limits of penetration and cone angle as descriptors; ECN benchmark studies establish the principle that optically defined metrics are method-dependent and must be reported as such; and recent data-driven work confirms that neural-network surrogates can reproduce spray macroscopic behavior at practical accuracy. What the existing literature does not address in combination is: (1) a pilot injection regime in which SOI and EOI transients dominate the observable window; (2) training targets that are systematically right-censored by a limited field-of-view chamber; (3) a per-plume extraction pipeline designed for multihole nozzle geometry; and (4) an integrated multi-stage learning framework that couples deterministic baseline, heteroscedastic uncertainty quantification, and distillation-based refinement. The present thesis addresses those four constraints as a unified design, implementing an end-to-end workflow from terabyte-scale Mie video to engineering-ready surrogate output.
...
\begin{table}[t]
\centering
\begin{tabular}{|L{0.21\linewidth}|C:\Users\LJI008\Mie_Postprocessing_Py\Thesis\mie_video_to_1d_dataflow.texL{0.26\linewidth}|L{0.33\linewidth}|}
\hline
...
\]
and the same routine also estimates an average angular offset from the clicked rim points relative to an ideal equally spaced set. \texttt{GUI.py} writes these values to \texttt{config.json}, after which \texttt{mie_multi_hole.py} can process a full folder without manual re-centering. This one-time calibration is deliberately user-facing rather than fully automatic: the operator can zoom, choose a representative frame, and anchor the full batch with a physically meaningful nozzle center \cite{Pratt1987CircleFit,Taubin1991CircleFit}.
...
\]
This is what makes the alignment self-adaptive: the code uses the observed harmonic phase of the current dataset instead of assuming a manually typed reference angle for every recording. \texttt{mie_multi_hole.py} then constructs plume directions
\[
...
\]
with \(q\) close to one. The code then enforces monotonicity by \texttt{np.maximum.accumulate}, subtracts the inner radius to express distance from the nozzle annulus, and in \texttt{mie_multi_hole.py} discards traces that show non-physical backward jumps or only very short valid runs. This CDF front is one of the main practical contributions of the workflow: it is much less sensitive to broken leading-edge pixels than a naive max-index definition, while still producing a penetration quantity aligned with standard diesel-spray usage \cite{Dent1971Penetration,HiroyasuArai1990,NaberSiebers1996}.
...
\begin{itemize}
  ...
  \item batch plume-wise feature extraction in \texttt{mie_multi_hole.py},
