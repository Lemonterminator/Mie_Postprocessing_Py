...
to yield a continuous collision probability rather than a binary pass/fail flag.

% Across roughly 15,000 penetration trajectories drawn from 9 nozzle campaigns (about 400,000 labelled time steps), the refined surrogate reaches an overall RMSE of 6.9~mm --- about 7.2\% of the measured penetration range --- and its heteroscedastic intervals stay close to nominal coverage (68.5\% at 1$\sigma$, 95.7\% at 2$\sigma$). The intent should be read plainly: this is a front-end engineering screening tool, not a substitute for high-fidelity in-cylinder physics. Within its declared scope --- non-reacting, room-temperature data and a simplified collision geometry --- the workflow is repeatable and auditable, and it provides a defensible base for later physically regularised or higher-dimensional extensions.
Across roughly 15,000 penetration trajectories drawn from 9 nozzle campaigns 
(about 400,000 labelled time steps), the refined surrogate reaches an overall RMSE of 6.9~mm 
--- about 7.2\% of the measured penetration 
--- and its heteroscedastic intervals stay close to nominal coverage 
(68.5\% at 1$\sigma$, 95.7\% at 2$\sigma$) against raw observables. 
...
\end{cases}
\]
Read in plain language, this says that one piece of the penetration law lives on the clock counted from start of injection, while another piece lives on the clock counted from end of injection --- or, more precisely, from the moment the decelerating entrainment-wave state starts to dominate at the tip. That distinction is genuinely useful here. It makes clear that the power-law exponent alone is not the full story: the effective time origin carries physical meaning of its own. It also makes the case for a softer engineering parameterisation, because in short pilot injections the observation window can contain both clocks at once and the precise switching instant is not directly readable from a noisy Mie-derived trace.

The collapse of the quasi-steady picture is not the fault of fluid dynamics alone. The upstream injector hardware adds its own dynamics. In a common-rail injection system, needle-valve opening and closing are finite-time mechanical transients; until the needle is fully lifted, the effective nozzle flow area varies with time and directly modulates the instantaneous injection rate and near-nozzle flow field \cite{StumppRicco1996,HiroyasuArai1990}. At high injection pressures, internal cavitation inside the nozzle can further alter the jet structure and atomisation characteristics in ways that classical penetration correlations make no attempt to capture \cite{BohrenHuffman2004}. Both effects are especially pronounced for short pilot injections, where opening and closing transients can swallow a substantial fraction of the total injection event and the quasi-steady assumption survives, at best, in a narrow middle window. The practical upshot for a model-builder is unflattering for rigid power-law correlations: data-driven models, freed from a specific model-form, do not require the transient regimes to be identified and parameterised in advance, and they can absorb the combined effect of several overlapping physical mechanisms without bolting together a piecewise rule for each one.
...
This structure is not a curiosity --- it is the entire point computationally. GPUs are particularly well matched to workloads dominated by regular dense-array operations, stencil-like neighbourhood computations, and repeated map-reduce style kernels, because the same arithmetic pattern is applied to many data elements in parallel \cite{NickollsBuckGarlandSkadron2008}. The individual operators used here are nothing exotic; they are standard image-processing primitives \cite{GonzalezWoodsDIP,Szeliski2010Vision}. The engineering twist is what happens once they are applied to terabyte-scale 12-bit video archives: the cumulative cost dominates the full workflow unless the operators are organised as bulk data-parallel computations.

GPU acceleration in this thesis is therefore not an implementation preference dressed up as a method choice. It is methodological. A serial frame-by-frame workflow can make even simple filtering and remapping steps too slow and too fragile for iterative model development; a tensor-oriented CUDA pipeline turns the same processing chain into something that can be rerun as calibration choices, target definitions, and model stages evolve. The blunt summary is that high-throughput processing is what moves this workflow from practically non-iterable to practically iterable --- and an iterable workflow is the difference between a thesis that can be revised and one that cannot.

\subsection{Censoring, truncation, and stable target construction}

A finite field of view does more than crop the image. It changes the statistics. When true late-time penetration extends past the visible window, the recorded trajectory is not just noisy --- it is right-censored by the observation limit. The structure is familiar from the broader literature on limited or censored dependent variables, where the observed value is only a truncated stand-in for the latent quantity of interest \cite{Tobin1958,BuckleyJames1979}. Both of those references originate in econometrics, which may seem far afield, but the core statistical pattern --- a dependent variable observable only below a threshold, with everything above suppressed --- is formally identical to what happens here: the true penetration crosses the field-of-view boundary, and the apparent terminal value is a property of the camera, not of the spray.
...
This distinction has direct teeth for surrogate-model training. Treat a censored trajectory's visible endpoint as an ordinary regression label and the target is biased downward in exactly the region where long-range penetration should still be growing. The learner is then quietly encouraged to read the spray's exit from the image as physical saturation, which it is not. In practice, mixing censored and uncensored trajectories in the same training set under this setup produces unstable late-time gradients and an artificial flattening that propagates through every downstream prediction.
...
Fuel-imaging literature has also been blunt about something easy to forget: the numerical definition of a spray metric is itself a modelling decision. Magnotti and Genzale showed that different liquid-penetration metrics can lead to materially different conclusions when validating diesel spray models, because different observables respond to different aspects of the underlying liquid distribution \cite{MagnottiGenzale2013}. The implication for Mie processing is uncomfortable but inescapable: a reported ``penetration'' value is not a universal truth but the output of a measurement rule applied to an optical signal --- change the rule, change the value.
...
The result is a workflow that makes no pretence of \emph{recovering physics} from intensity. It uses image-processing theory and prior spray-imaging evidence to construct reproducible, physically interpretable geometric observables and states their limitations openly --- which is, in the author's view, the only honest stance available given the diagnostic.

The Engine Combustion Network (ECN) benchmark work has documented the method-dependence of spray measurements perhaps more systematically than anyone else. By testing nominally identical injectors (Spray~A and Spray~D) across multiple institutions, ECN found measurable inter-facility variation in liquid-penetration values, and concluded explicitly that optically measured liquid boundaries should be understood as quantities defined by a particular measurement protocol, not as absolute ground-truth representations of the liquid phase \cite{ecn_liquid_penetration,ecn_liquid_penetration_accessory,MeijerECN2012}. The experimental context here differs from ECN Spray~A (high-temperature, high-pressure evaporating conditions) and Spray~D (multihole nozzle) --- the present data come from a room-temperature pressurised chamber with a multihole marine nozzle and no fuel evaporation. But the ECN methodological principle still bites: the numerical values of reported penetration and boundary metrics depend on the post-processing rules, and that dependency has to be on the page when results are interpreted.
...
For short-pulse pilot injections, that deterministic threshold is, in the author's view, the wrong instrument for the job. Penetration here carries non-negligible uncertainty from needle-valve transients, cycle-to-cycle fluctuations in delivered momentum, and residual optical-measurement noise, and the SOI/EOI-dominated regime makes steady-state correlations imprecise over a large fraction of the observable window. A binary pass/fail rule, applied on top of a quantity that is itself a distribution rather than a number, hides exactly the information a designer would want to see. The framework proposed in this thesis treats the issue head-on: penetration is represented as a conditional Gaussian distribution $S_\mathrm{tip}\sim\mathcal{N}(\mu(x,t),\,\sigma^2(x,t))$, and that distribution is projected onto a two-dimensional engine-geometry plane centred on the nozzle. With the lateral spread modelled as a normal distribution governed by the spray half-cone angle, the spray probability density function takes the form of a rotated two-dimensional Gaussian, and impingement risk reduces to the probability-mass integral of this distribution over the geometric wall and piston masks. The output is a continuous collision probability rather than a binary verdict --- which is what an early-stage design sweep actually needs in order to rank candidate conditions, not merely accept or reject them.

Pulled together, the strands of literature surveyed above leave the following foundations standing for the present work. Mie scattering theory provides a defensible mapping from optical intensity to the geometric observables actually used downstream. Computer vision and signal processing theory underwrites each step of the image-processing chain individually, even where no single off-the-shelf tool was the right fit for the whole pipeline. Diesel spray physics fixes the physical meaning of penetration and cone angle as descriptors and, just as importantly, fixes the limits of what those descriptors can claim to represent. The ECN benchmark studies have made it clear that optically defined metrics are method-dependent and must be reported as such, rather than treated as ground truth. Recent data-driven work has settled the practical question of whether neural-network surrogates can reproduce spray macroscopic behaviour at useful accuracy, leaving open the question of how they should behave when the supervision itself is partial. And the spray-wall impingement literature has converged on penetration depth as the central geometric input for screening while quietly exposing how brittle the deterministic threshold rule becomes under strongly transient, high-uncertainty conditions. What no single thread in this literature addresses in combination, however, is the joint problem this thesis takes on: (1) a pilot injection regime in which SOI and EOI transients dominate the observable window; (2) training targets that are systematically right-censored by a limited field-of-view chamber; (3) a per-plume extraction pipeline designed around multihole nozzle geometry rather than retrofitted to it; (4) an integrated multi-stage learning framework that couples a deterministic baseline, heteroscedastic uncertainty quantification, and distillation-based refinement into a single training schedule; and (5) the direct mapping of the conditional Gaussian penetration output onto engine geometry masks to produce a probabilistic collision risk suitable for early-stage design screening. Addressing those five constraints as a unified design, rather than as five loosely coupled studies, is the contribution that the rest of this thesis attempts to defend.
