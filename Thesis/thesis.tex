...
That is exactly the role of the curve fit step in the workflow, %\texttt{fit\_raw\_data.py},
which aligns penetration time-series to SOI individually,
then uses a robust nonlinear fit to convert noisy plume traces into compact parameters for the later learning stages.
...
Heteroscedastic Gaussian negative log likelihood is the standard way to let a network predict both a mean and an input-dependent variance 
\cite{KendallGal2017}. 
...
When both teacher and student outputs are represented as univariate Gaussians, the Kullback--Leibler divergence between them 
...
An up-to-date breakdown is regenerated automatically by \texttt{MLP/summarize\_dataset.py}
at the end of every \texttt{fit\_raw\_data.py} run.
...
\textbf{Piston geometry dispatcher.}  The notebook exposes two interchangeable crown profiles in \codepath{MLP/piston/}. Design~A (\codepath{design\_A.py}) assembles a bowl-shaped crown from five circular-arc radii --- outer bowl, inner bowl, topland, lip, and floor --- with hard closure constraints that keep the geometry consistent when any radius is changed. The Hermite variant (\codepath{design\_hermite.py}) replaces the piecewise-arc construction with a \emph{quintic Hermite spline} defined by a small set of control points, each carrying a position, a tangent vector, and a second-derivative vector in the (radial, depth) cross-section plane. Matching position, slope, and curvature at every knot gives $C^{2}$ continuity across segment boundaries, so the crown surface has no curvature jumps --- a property relevant both for CNC machining (where curvature discontinuities translate into tool-acceleration steps) and for the collision-integral mesh (where abrupt curvature changes could alias onto the rasterised mask).
...
(i)~the baseline configuration with $w_{\mathrm{kd}}=0.25$ in \texttt{raw\_reliable} bins and $\lambda_{\mathrm{anchor}}=0.01$;
(ii)~\texttt{raw\_reliable\_no\_kd}, which removes KD blending in reliable bins;
(iii)~\texttt{raw\_uncertain\_raw05\_kd025}, which reintroduces partial raw supervision in the uncertain regime; and
(iv)~\texttt{anchor\_off}, which sets $\lambda_{\mathrm{anchor}}=0$ while leaving the raw/KD regime design unchanged.
...
\midrule
\texttt{anchor\_off} & 6.68 & 4.83 & -0.02 & 13.89 \\
\texttt{raw\_reliable\_no\_kd} & 6.82 & 5.01 & -0.91 & 14.17 \\
\texttt{raw\_uncertain\_raw05\_kd025} & 6.87 & 5.07 & -1.23 & 14.21 \\
\texttt{baseline} & 6.91 & 5.08 & -1.37 & 14.32 \\
\bottomrule
\end{tabular}
\caption{Stage~3 ablation ranking from the automatic post-training RMSE evaluation. The selected model is \texttt{anchor\_off}.}
