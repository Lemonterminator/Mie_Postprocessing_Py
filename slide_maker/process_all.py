...
Processes all .cine (Mie, Luminescence) and .dxd (Dewesoft) files
in the configured directories and saves results.

Schlieren videos are NOT processed here — pass them directly to
manual_segment.py after acquisition.
...
def process_video_files(config: dict, video_type: str) -> None:
    """Process all .cine files for a given video type (mie or luminescence).

    Schlieren is intentionally excluded — handled by manual_segment.py.
    """
    if video_type.lower() == "schlieren":
        print("Schlieren: skipped here — use manual_segment.py instead.")
        return
...
def main():
    ...
    for video_type in ["mie", "luminescence"]:
