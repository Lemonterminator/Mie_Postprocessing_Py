...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--video-path", required=True, help="Local JPEG frame directory or MP4 file.")
    parser.add_argument(
        "--checkpoint",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Probability threshold used for exported binary masks. Default: 0.5",
    )
    parser.add_argument(
        "--max-frames",
        type=int,
...
def resolve_frame_paths(video_path: Path) -> list[Path]:
    if video_path.is_dir():
        return sorted(
            [p for p in video_path.iterdir() if p.is_file() and p.suffix.lower() in {".jpg", ".jpeg", ".png"}]
        )
    raise ValueError("Only JPEG/PNG frame directories are supported for export in this script.")
...
def main() -> int:
    ...
    if not frame_paths:
        return fail(f"no image frames found in: {video_path}")
    ...
    print(f"[ok] add_prompt_keys={sorted(response['outputs'].keys())}")
    ...
    try:
        for frame_counter, streamed_output in enumerate(
            predictor.handle_stream_request({"type": "propagate_in_video", "session_id": session_id}),
            start=1,
        ):
            ...
            frame_stats = to_jsonable(dict(outputs.get("frame_stats", {})))
            frame_name = frame_paths[frame_index].stem

            if probs.ndim >= 1 and masks.shape[0] == probs.shape[0]:
                keep = probs >= float(args.mask_threshold)
            ...
            if save_overlay:
                frame_rgb = load_frame_rgb(frame_paths[frame_index])
            ...
            if frame_counter == 1:
                print(f"[ok] first_frame_record={record}")
